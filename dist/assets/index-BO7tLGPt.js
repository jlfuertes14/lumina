var me=(e,t)=>()=>(t||e((t={exports:{}}).exports,t),t.exports);var je=me((Re,N)=>{(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))r(a);new MutationObserver(a=>{for(const n of a)if(n.type==="childList")for(const l of n.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&r(l)}).observe(document,{childList:!0,subtree:!0});function i(a){const n={};return a.integrity&&(n.integrity=a.integrity),a.referrerPolicy&&(n.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?n.credentials="include":a.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function r(a){if(a.ep)return;a.ep=!0;const n=i(a);fetch(a.href,n)}})();const pe="http://localhost:3000",ue="https://lumina-jlfuertes14s-projects.vercel.app";function ge(e){const t=e.trim();return t?t.replace(/\/api\/?$/i,"").replace(/\/+$/,""):""}const ie=ge("https://lumina-backend-jr2a.onrender.com"),ve=ie||ue,j=window.location.hostname,re=j==="localhost"||j==="127.0.0.1",ae=j.endsWith(".onrender.com"),ne=j.endsWith(".vercel.app"),se=ae||ne,G=re?pe:se?window.location.origin:ve,V=se?"/api":`${G}/api`,he=G,fe=re?"DEVELOPMENT":ae?"RENDER":ne?"VERCEL":"PRODUCTION",ye=ie?"VITE_BACKEND_ORIGIN":"DEFAULT_PRODUCTION_BACKEND_ORIGIN";let de=class{constructor(t=he){this.serverUrl=t,this.socket=null,this.connected=!1,this.currentDeviceId=null,this.eventHandlers=new Map}async connect(t,i=""){if(this.socket&&this.connected){console.warn("Already connected to WebSocket");return}return new Promise((r,a)=>{if(typeof io>"u"){const n=document.createElement("script");n.src="https://cdn.socket.io/4.5.4/socket.io.min.js",n.onload=()=>this._initializeSocket(t,i,r,a),n.onerror=()=>a(new Error("Failed to load Socket.IO")),document.head.appendChild(n)}else this._initializeSocket(t,i,r,a)})}_initializeSocket(t,i,r,a){try{this.socket=io(`${this.serverUrl}/control`,{auth:{userId:t,sessionToken:i},transports:["websocket","polling"]}),this.socket.on("connect",()=>{console.log("✅ Connected to ESP32 WebSocket server"),this.connected=!0,this._triggerEvent("connected"),r()}),this.socket.on("connect_error",n=>{console.error("❌ Connection error:",n.message),this.connected=!1,this._triggerEvent("error",n),a(n)}),this.socket.on("disconnect",()=>{console.log("❌ Disconnected from WebSocket server"),this.connected=!1,this._triggerEvent("disconnected")}),this.socket.on("device:status",n=>{console.log("📊 Device status:",n),this._triggerEvent("device:status",n)}),this.socket.on("device:telemetry",n=>{this._triggerEvent("device:telemetry",n)}),this.socket.on("command:response",n=>{this._triggerEvent("command:response",n)}),this.socket.on("command:sent",n=>{this._triggerEvent("command:sent",n)}),this.socket.on("device:error",n=>{console.error("Device error:",n),this._triggerEvent("device:error",n)}),this.socket.on("devices:list",n=>{this._triggerEvent("devices:list",n)}),this.socket.on("error",n=>{console.error("System error:",n),this._triggerEvent("error",n)})}catch(n){a(n)}}monitorDevice(t){if(!this.connected)throw new Error("Not connected to WebSocket server");this.currentDeviceId=t,this.socket.emit("monitor:device",t),console.log(`👁️ Monitoring device: ${t}`)}stopMonitoring(){this.currentDeviceId&&this.connected&&(this.socket.emit("monitor:stop",this.currentDeviceId),this.currentDeviceId=null)}sendCommand(t,i,r={}){if(!this.connected)throw new Error("Not connected to WebSocket server");this.socket.emit("control:command",{deviceId:t,command:i,payload:r}),console.log(`📤 Sent command to ${t}:`,i,r)}move(t,i,r=255){this.sendCommand(t,"move",{direction:i,speed:r})}stop(t){this.sendCommand(t,"stop")}turnLeft(t,i=200){this.sendCommand(t,"move",{direction:"left",speed:i})}turnRight(t,i=200){this.sendCommand(t,"move",{direction:"right",speed:i})}forward(t,i=255){this.sendCommand(t,"move",{direction:"forward",speed:i})}backward(t,i=255){this.sendCommand(t,"move",{direction:"backward",speed:i})}requestDeviceList(){if(!this.connected)throw new Error("Not connected to WebSocket server");this.socket.emit("devices:list")}on(t,i){this.eventHandlers.has(t)||this.eventHandlers.set(t,[]),this.eventHandlers.get(t).push(i)}off(t,i){if(!this.eventHandlers.has(t))return;const r=this.eventHandlers.get(t),a=r.indexOf(i);a>-1&&r.splice(a,1)}_triggerEvent(t,i=null){if(!this.eventHandlers.has(t))return;this.eventHandlers.get(t).forEach(a=>{try{a(i)}catch(n){console.error(`Error in event handler for ${t}:`,n)}})}disconnect(){this.socket&&(this.stopMonitoring(),this.socket.disconnect(),this.socket=null,this.connected=!1,console.log("👋 Disconnected from WebSocket"))}isConnected(){return this.connected}getCurrentDevice(){return this.currentDeviceId}};typeof N<"u"&&N.exports&&(N.exports=de);typeof window<"u"&&(window.ESP32SocketClient=de);async function S(e,t={}){const i=`${V}${e}`;try{const r=await fetch(i,{...t,headers:{"Content-Type":"application/json",...t.headers}}),a=await r.json();if(!r.ok)throw new Error(a.error||a.message||"API request failed");return a}catch(r){throw console.error("API Error:",r),r}}console.log(`🌍 Environment: ${fe}`);console.log(`🧭 Backend origin (${ye}): ${G}`);console.log(`🔗 API URL: ${V}`);const we=({Breadcrumbs:e})=>{const t=[{name:"Marco Dela Cruz",role:"Founder & CEO",image:"https://ui-avatars.com/api/?name=Marco+Dela+Cruz&background=6366f1&color=fff&size=120"},{name:"Rina Gonzales",role:"Operations Manager",image:"https://ui-avatars.com/api/?name=Rina+Gonzales&background=6366f1&color=fff&size=120"},{name:"Luis Navarro",role:"Technical Support Engineer",image:"https://ui-avatars.com/api/?name=Luis+Navarro&background=6366f1&color=fff&size=120"},{name:"Ella Ramirez",role:"Customer Service Lead",image:"https://ui-avatars.com/api/?name=Ella+Ramirez&background=6366f1&color=fff&size=120"}];return`
    ...
    `,y=e=>new Intl.NumberFormat("en-PH",{style:"currency",currency:"PHP"}).format(e),Ce=({Breadcrumbs:e,state:t})=>{const i=t.products.slice(0,8).map((r,a)=>({...r,originalPrice:r.price*1.3,discount:[10,15,20,25,30][a%5],category:["Microcontrollers","Sensors","Tools","Robotics","Components"][a%5]}));return t.myCoupons,`
        <div style="background: #f8f9fa; min-height: 100vh; padding: 2rem 0;">
            <!-- Content Container -->
            <div style="max-width: 1400px; margin: 0 auto; padding: 0 2rem;">
                ${e("deals")}
    ...
    `,c=()=>{const{productSearch:x,productCategoryFilter:p,productSortBy:w,productSortOrder:E}=window.adminState,O=[...new Set(e.products.map(k=>k.category))].sort(),D=k=>{const A={Microcontrollers:"MIC",Sensors:"SEN",Components:"COM",Robotics:"ROB","Cables & Wires":"CAB","3D Printing":"3DP",Power:"PWR",Displays:"DIS",Modules:"MOD"}[k.category]||k.category?.substring(0,3).toUpperCase()||"GEN",H=(k._id||k.id||"").toString().slice(-4).toUpperCase();return`LUM-${A}-${H}`};let U=e.products.filter(k=>{const z=x.toLowerCase(),A=D(k).toLowerCase(),H=x===""||k.name.toLowerCase().includes(z)||(k._id||k.id).toString().toLowerCase().includes(z)||A.includes(z),ce=p==="all"||k.category===p;return H&&ce});w&&(U=[...U].sort((k,z)=>{let A=0;switch(w){case"price":A=k.price-z.price;break;case"stock":A=k.stock-z.stock;break;case"status":A=(z.stock>0?1:0)-(k.stock>0?1:0);break}return E==="asc"?A:-A}));const q=k=>w!==k?"⇅":E==="asc"?"↑":"↓",R=k=>w===k?"color: #3b82f6; font-weight: 600;":"";return`
        <div class="admin-page-header">
        ...
        <div style="margin-bottom: 0.75rem; color: #64748b; font-size: 0.875rem;">
            Showing ${U.length} of ${e.products.length} products
            ${x?` matching "${x}"`:""}
            ${p!=="all"?` in ${p}`:""}
        </div>
            ...
            <table class="admin-table">
                <thead>
                    <tr>
                        <th>Product</th>
                        <th>Category</th>
                        <th onclick="window.handleProductSort('price')" style="cursor: pointer; user-select: none; ${R("price")}">
                            Price <span style="font-size: 0.75rem; opacity: 0.7;">${q("price")}</span>
                        </th>
                        <th onclick="window.handleProductSort('stock')" style="cursor: pointer; user-select: none; ${R("stock")}">
                            Stock <span style="font-size: 0.75rem; opacity: 0.7;">${q("stock")}</span>
                        </th>
                        <th onclick="window.handleProductSort('status')" style="cursor: pointer; user-select: none; ${R("status")}">
                            Status <span style="font-size: 0.75rem; opacity: 0.7;">${q("status")}</span>
                        </th>
                        <th>Actions</th>
                    </tr>
                </thead>
    ...
    `},$=()=>{const x=[...e.orders].sort((p,w)=>new Date(w.createdAt)-new Date(p.createdAt));return`
        <div class="admin-page-header">
                        ...
                        <tr>
                            ...
                            <td>
                                <div style="display: flex; align-items: center; gap: 0.5rem;">
                                    <div class="admin-avatar-circle" style="width: 28px; height: 28px; font-size: 0.75rem;">${E.charAt(0).toUpperCase()}</div>
                                    <span>${E}</span>
                                </div>
                            </td>
                            <td>
    ...
    `},I=()=>{if(!window.adminState.isModalOpen)return"";let x="",p="";if(window.adminState.modalType==="addProduct"||window.adminState.modalType==="editProduct"){const w=window.adminState.editingId?e.products.find(E=>String(E._id||E.id)===String(window.adminState.editingId)):null;x=w?"Edit Product":"Add New Product",p=`
            ...
            `}else if(window.adminState.modalType==="addStaff"||window.adminState.modalType==="editStaff"){const w=window.adminState.editingId?e.users.find(E=>String(E.id)===String(window.adminState.editingId)||E._id===window.adminState.editingId):null;x=w?"Edit Staff Member":"Add Staff Member",p=`
    ...
    `},$e=e=>{const t=e.stock<10,i=t?"low-stock":"",r=t?"Low Stock":"In Stock";return`
    ...
    `},ee=()=>{if(o.searchQuery){const i=o.products.filter(r=>r.name.toLowerCase().includes(o.searchQuery.toLowerCase())||r.category.toLowerCase().includes(o.searchQuery.toLowerCase())||r.description.toLowerCase().includes(o.searchQuery.toLowerCase()));return`
            <div class="hero">
    ...
    `},Ee=()=>{if(!o.currentUser)return P("login"),"";const e=o.devices||[];return`
    ...
    `:(P("login"),""),Le=()=>{if(!o.currentUser)return P("login"),"";if(!o.currentDeviceId)return P("my-devices"),"";const e=o.devices?.find(n=>n.deviceId===o.currentDeviceId);if(!e)return P("my-devices"),"";const t=o.deviceStatus[e.deviceId]||{},i=o.telemetryData[e.deviceId]||{},r=t.status==="online",a=i.obstacleAvoidance||!1;return i.speed,`
    ...
    `},Oe=()=>{if(!o.lastOrderId)return P("home"),"";const e=o.orders.find(n=>n.orderId===o.lastOrderId);if(!e)return P("home"),"";const t=new Date,i=new Date(t);i.setDate(t.getDate()+3);const r=new Date(t);r.setDate(t.getDate()+5);const a=`${i.toLocaleDateString("en-US",{month:"short",day:"numeric"})} - ${r.toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"})} `;return`
    ...
    `,a=window.open("","_blank","width=300,height=600");a.document.write(r),a.document.close(),a.onload=function(){setTimeout(()=>{a.print()},250)}};window.handleSearchInput=e=>{const t=e.target.value;if(o.searchQuery=t,t.trim()){const i=new Set;o.products.forEach(r=>{const a=r.name.toLowerCase(),n=r.category.toLowerCase(),l=t.toLowerCase();a.includes(l)&&i.add(r.name),n.includes(l)&&i.add(r.category),a.split(" ").forEach(d=>{d.toLowerCase().startsWith(l)&&d.length>2&&i.add(d.charAt(0).toUpperCase()+d.slice(1))})}),o.searchSuggestions=Array.from(i).slice(0,8),o.showSuggestions=!0}else if(o.searchSuggestions=[],o.showSuggestions=!1,o.route==="home"||o.route==="products"){C();return}le()};function le(){const e=document.querySelector(".search-container");if(!e)return;const t=e.querySelector(".search-suggestions");if(t&&t.remove(),o.showSuggestions&&o.searchQuery){const i=`
    ...
    `,document.body.appendChild(a);const n=l=>{l.key==="Escape"&&(a.remove(),document.removeEventListener("keydown",n))};document.addEventListener("keydown",n)};const C=async()=>{const e=document.getElementById("app");let t="";switch(o.route){case"home":t=ee();break;case"products":t=Se({Breadcrumbs:L,state:o});break;case"product-detail":t=Pe({Breadcrumbs:L,state:o});break;case"signup":t=Me();break;case"cart":t=Te();break;case"checkout":t=ze();break;case"order-confirmation":t=Oe();break;case"contact-us":t=ke({Breadcrumbs:L});break;case"about-us":t=we({Breadcrumbs:L});break;case"learn":t=be({Breadcrumbs:L,state:o});break;case"tutorial-detail":t=xe({Breadcrumbs:L,state:o});break;case"deals":t=Ce({Breadcrumbs:L,state:o});break;case"admin":t=Y(o);break;case"user":t=X({state:o});break;case"admin-profile":t=K({state:o});break;case"my-devices":t=Ee();break;case"device-pair":t=Ae();break;case"remote-control":t=Le();break;default:t=ee()}if(o.route==="admin"){e.innerHTML=Y(o),window.initAdminCharts&&setTimeout(()=>window.initAdminCharts(),100);return}if(o.route==="admin-profile"){e.innerHTML=Q()+K({state:o});return}if(o.route==="user"){o.currentUser&&o.orders.length===0&&await M.getMyOrders(),o.currentUser&&!o.myCoupons&&await M.getMyCoupons(),e.innerHTML=Q()+X({state:o});return}e.innerHTML=`
        ${Q()}
        <main>
            ${t}
        </main>
    ...
    `,document.body.appendChild(t),setTimeout(()=>t.classList.add("show"),10)};const Ue=async()=>{const t=new URLSearchParams(window.location.search).get("product_id"),i=sessionStorage.getItem("currentProductId");t?(o.currentProductId=parseInt(t),sessionStorage.setItem("currentProductId",o.currentProductId),o.route="product-detail"):i&&(o.currentProductId=parseInt(i)),await M.getProducts(),o.currentUser?.role==="admin"?await Promise.all([M.getOrders(),B.getMyDevices(),M.getUsers()]):o.currentUser&&await B.getMyDevices(),C()};window.showLoading=()=>{let e=document.querySelector(".loading-overlay");e||(e=document.createElement("div"),e.className="loading-overlay",e.innerHTML='<div class="spinner"></div>',document.body.appendChild(e)),e.offsetWidth,e.classList.add("active")};window.hideLoading=()=>{const e=document.querySelector(".loading-overlay");e&&(e.classList.remove("active"),setTimeout(()=>{e.parentNode&&e.parentNode.removeChild(e)},300))};window.openLoginModal=()=>{document.body.insertAdjacentHTML("beforeend",`
