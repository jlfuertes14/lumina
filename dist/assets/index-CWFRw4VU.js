(function(){const a=document.createElement("link").relList;if(a&&a.supports&&a.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))n(i);new MutationObserver(i=>{for(const o of i)if(o.type==="childList")for(const d of o.addedNodes)d.tagName==="LINK"&&d.rel==="modulepreload"&&n(d)}).observe(document,{childList:!0,subtree:!0});function s(i){const o={};return i.integrity&&(o.integrity=i.integrity),i.referrerPolicy&&(o.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?o.credentials="include":i.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function n(i){if(i.ep)return;i.ep=!0;const o=s(i);fetch(i.href,o)}})();const O="https://lumina-production-a4bb.up.railway.app",M="http://localhost:3000",D=window.location.hostname==="jlfuertes14.github.io",P=D?`${O}/api`:`${M}/api`;async function b(e,a={}){const s=`${P}${e}`;try{const n=await fetch(s,{...a,headers:{"Content-Type":"application/json",...a.headers}}),i=await n.json();if(!n.ok)throw new Error(i.error||i.message||"API request failed");return i}catch(n){throw console.error("API Error:",n),n}}console.log(`🌍 Environment: ${D?"PRODUCTION":"DEVELOPMENT"}`);console.log(`🔗 API URL: ${P}`);const t={products:[],users:[],orders:[],currentUser:JSON.parse(localStorage.getItem("currentUser"))||null,cart:JSON.parse(localStorage.getItem("cart_v2"))||[],route:sessionStorage.getItem("currentRoute")||"home",mobileMenuOpen:!1,searchQuery:"",showSuggestions:!1,searchSuggestions:[],currentProductId:null,sortBy:"featured",filterCategory:null,cartSearchQuery:"",isLoading:!1,cartSynced:!1,checkoutData:{shipping:{fullName:"",address:"",city:"",province:"",postalCode:"",phone:"",instructions:""},paymentMethod:"cod",shippingFee:50},lastOrderId:null},f={getProducts:async()=>{try{const e=await b("/products");t.products=e.data,g()}catch(e){console.error("Failed to load products:",e),c()}},login:async(e,a)=>{console.log("Calling API login...");try{const s=await b("/users/login",{method:"POST",body:JSON.stringify({email:e,password:a})});t.currentUser=s.data,localStorage.setItem("currentUser",JSON.stringify(t.currentUser)),await E(),c(`Welcome back, ${t.currentUser.name}!`),y("home")}catch(s){throw c(s.message||"Login failed"),s}},register:async(e,a,s)=>{try{const n=await b("/users/register",{method:"POST",body:JSON.stringify({name:e,email:a,password:s})});t.currentUser=n.data,localStorage.setItem("currentUser",JSON.stringify(t.currentUser)),c("Account created successfully!"),y("home")}catch(n){throw c(n.message||"Registration failed"),n}},createOrder:async e=>{try{const a=await b("/orders",{method:"POST",body:JSON.stringify(e)});return t.cart=[],w(),c("Order placed successfully!"),t.currentUser.role==="admin"&&f.getOrders(),a.data}catch(a){throw c(a.message||"Failed to place order"),a}},getOrders:async()=>{if(!(!t.currentUser||t.currentUser.role!=="admin"))try{const e=await b("/orders");t.orders=e.data,g()}catch(e){console.error("Failed to load orders:",e)}},getUsers:async()=>{if(!(!t.currentUser||t.currentUser.role!=="admin"))try{const e=await b("/users");t.users=e.data,g()}catch(e){console.error("Failed to load users:",e)}}},w=()=>{localStorage.setItem("currentUser",JSON.stringify(t.currentUser)),localStorage.setItem("cart_v2",JSON.stringify(t.cart))},E=async()=>{if(!(!t.currentUser||t.cartSynced))try{const e=await b(`/users/${t.currentUser.id}/cart/sync`,{method:"POST",body:JSON.stringify({localCart:t.cart})});t.cart=e.data.map(a=>({id:a.productId,name:a.name,price:a.price,image:a.image,quantity:a.quantity,category:a.category,selected:a.selected})),t.cartSynced=!0,w(),g(),c("Cart synced!")}catch(e){console.error("Cart sync failed:",e)}},l=e=>new Intl.NumberFormat("en-PH",{style:"currency",currency:"PHP"}).format(e),y=e=>{t.route=e,t.mobileMenuOpen=!1,sessionStorage.setItem("currentRoute",e),g(),window.scrollTo(0,0)},c=e=>{const a=document.createElement("div");return a.className="toast",`
        <header>
                        ...
                        <div class="search-container">
                            ...
                            ${t.showSuggestions&&t.searchQuery?`
                                <div class="search-suggestions" id="searchSuggestions">
                                    ...
                                    ${t.searchSuggestions.slice(0,5).map(s=>`
            ...
            <nav class="header-nav ${t.mobileMenuOpen?"mobile-open":""}">
                ...
                ${isAdmin?`<a href="#" class="nav-link ${t.route==="admin"?"active":""}" onclick="window.navigate('admin'); return false;">Admin Dashboard</a>`:""}
            </nav>
        </header>
    ...
    `},B=()=>{const e=t.products.find(i=>i.id===t.currentProductId);if(!e)return y("home"),"";e.stock<10;const a=e.stock>0?"In Stock":"Out of Stock",s=e.stock>0?"var(--success)":"var(--danger)",n=t.products.filter(i=>i.id!==e.id).sort(()=>.5-Math.random()).slice(0,4);return`
    ...
    `},q=()=>{if(!t.currentUser)return y("login"),"";const e=t.cart.filter(i=>i.selected!==!1);if(e.length===0)return c(),y("cart"),"";const a=e.reduce((i,o)=>i+o.price*o.quantity,0),s=t.checkoutData.shippingFee,n=a+s;return`
    ...
    `};window.updateOrderStatus=(e,a)=>{const s=t.orders.find(n=>n.id===e);s&&(s.status=a,w(),c(),g())};window.toggleSelectAll=e=>{document.querySelectorAll(".product-checkbox").forEach(s=>s.checked=e.checked)};window.bulkAction=e=>{const a=document.querySelectorAll(".product-checkbox:checked"),s=Array.from(a).map(n=>parseInt(n.value));if(s.length===0){c();return}e==="delete"?confirm(`Delete ${s.length} products?`)&&(t.products=t.products.filter(n=>!s.includes(n.id)),w(),g(),c()):e==="restock"&&(t.products.forEach(n=>{s.includes(n.id)&&(n.stock+=10)}),w(),g(),c())};window.viewOrderDetails=e=>{const a=t.orders.find(i=>i.id===e);if(!a)return;const s=t.users.find(i=>i.id===a.userId),n=`
    ...
    `};window.navigate=y;window.toggleMobileMenu=()=>{t.mobileMenuOpen=!t.mobileMenuOpen,g()};window.showProductModal=(e=null)=>{document.body.insertAdjacentHTML("beforeend",ProductModal(e))};window.closeProductModal=()=>{document.getElementById("productModal")?.remove()};window.editProduct=e=>{window.showProductModal(e)};window.deleteProduct=async e=>{if(confirm("Delete this product?"))try{await b(`/products/${e}`,{method:"DELETE"}),await f.getProducts(),c("Product deleted")}catch{c()}};window.handleProductSubmit=async(e,a)=>{e.preventDefault();const s=new FormData(e.target);try{const n=a?"PUT":"POST",i=a?`/products/${a}`:"/products";if(!(await fetch(`${P}${i}`,{method:n,body:s})).ok)throw new Error("Failed");await f.getProducts(),window.closeProductModal(),c(a?"Product updated!":"Product created!")}catch{c()}};window.handleSort=e=>{t.sortBy=e,g()};window.viewProduct=e=>{t.currentProductId=e,y("product-detail")};window.adjustDetailQty=e=>{const a=document.getElementById("detailQty");let s=parseInt(a.value)+e;s<1&&(s=1),a.value=s};window.addToCartFromDetail=e=>{const a=parseInt(document.getElementById("detailQty").value);if(!t.currentUser){c(),y("login");return}const s=t.products.find(i=>i.id===e),n=t.cart.find(i=>i.id===e);n?n.quantity+=a:t.cart.push({...s,quantity:a}),w(),c()};window.addToCart=async e=>{if(!t.currentUser){c(),y("login");return}const a=t.products.find(n=>n.id===e),s=t.cart.find(n=>n.id===e);s?s.quantity+=1:t.cart.push({...a,quantity:1}),w(),g(),c(),t.currentUser&&await b(`/users/${t.currentUser.id}/cart`,{method:"PUT",body:JSON.stringify({cart:t.cart.map(n=>({productId:n.id,name:n.name,price:n.price,image:n.image,quantity:n.quantity,category:n.category,selected:n.selected!==!1}))})})};window.updateQuantity=(e,a)=>{if(a<1){window.removeFromCart(e);return}const s=t.cart.find(n=>n.id===e);s&&(s.quantity=a,w(),g())};window.removeFromCart=e=>{t.cart=t.cart.filter(a=>a.id!==e),w(),g()};window.checkout=async()=>{if(t.cart.length===0)return;if(!t.currentUser){c(),y("login");return}if(t.cart.filter(a=>a.selected!==!1).length===0){c();return}y("checkout")};window.updateShippingInfo=(e,a)=>{t.checkoutData.shipping[e]=a};window.selectPaymentMethod=e=>{t.checkoutData.paymentMethod=e,g()};window.handlePhoneInput=e=>{const a=e.value.replace(/[^0-9]/g,"");e.value=a,t.checkoutData.shipping.phone=a};window.handleNameInput=e=>{const a=e.value.replace(/[^a-zA-Z\s]/g,"");e.value=a,t.checkoutData.shipping.fullName=a};window.handleLocationInput=(e,a)=>{const s=e.value.replace(/[^a-zA-Z\s]/g,"");e.value=s,t.checkoutData.shipping[a]=s};window.handlePostalInput=e=>{const a=e.value.replace(/[^0-9]/g,"");e.value=a,t.checkoutData.shipping.postalCode=a};window.showPaymentModal=e=>new Promise(a=>{let s="";const n=`
    ...
    `,i=window.open("","_blank","width=300,height=600");i.document.write(n),i.document.close(),i.onload=function(){setTimeout(()=>{i.print()},250)}};window.handleSearchInput=e=>{const a=e.target.value;if(t.searchQuery=a,a.trim()){const s=new Set;t.products.forEach(n=>{const i=n.name.toLowerCase(),o=n.category.toLowerCase(),d=a.toLowerCase();i.includes(d)&&s.add(n.name),o.includes(d)&&s.add(n.category),i.split(" ").forEach(p=>{p.toLowerCase().startsWith(d)&&p.length>2&&s.add(p.charAt(0).toUpperCase()+p.slice(1))})}),t.searchSuggestions=Array.from(s).slice(0,8),t.showSuggestions=!0}else if(t.searchSuggestions=[],t.showSuggestions=!1,t.route==="home"||t.route==="products"){g();return}L()};function L(){const e=document.querySelector(".search-container");if(!e)return;const a=e.querySelector(".search-suggestions");if(a&&a.remove(),t.showSuggestions&&t.searchQuery){const s=`
            <div class="search-suggestions" id="searchSuggestions">
                <div class="suggestions-header">Suggestions</div>
        ...
        `;e.insertAdjacentHTML("beforeend",s)}}window.showSearchSuggestions=()=>{t.searchQuery&&(t.showSuggestions=!0,L())};window.selectSuggestion=e=>{t.searchQuery=e,t.showSuggestions=!1;const a=document.getElementById("searchInput");a&&(a.value=e),handleSearch()};window.handleSearch=()=>{t.showSuggestions=!1;const e=document.getElementById("searchInput");e&&(t.searchQuery=e.value.trim()),y("products"),setTimeout(()=>{const a=document.querySelector(".product-grid");a&&a.scrollIntoView({behavior:"smooth",block:"start"})},100)};window.clearSearch=()=>{t.searchQuery="",t.showSuggestions=!1,t.searchSuggestions=[],g()};document.addEventListener("click",e=>{if(!e.target.closest(".search-container")&&t.showSuggestions){t.showSuggestions=!1;const a=document.querySelector(".search-suggestions");a&&a.remove()}});window.handleLogin=async e=>{console.log("Login attempt started"),e.preventDefault();const a=e.target.email.value,s=e.target.password.value;console.log("Credentials:",{email:a,password:s});try{await f.login(a,s),console.log("Login successful")}catch(n){console.error("Login error:",n)}};window.handleSignup=async e=>{console.log("Signup attempt started"),e.preventDefault();const a=e.target.name.value,s=e.target.email.value,n=e.target.password.value;try{await f.register(a,s,n)}catch(i){console.error("Signup error:",i)}};window.logout=()=>{t.currentUser=null,t.cart=[],localStorage.removeItem("currentUser"),localStorage.removeItem("cart_v2"),c(),sessionStorage.removeItem("currentRoute"),y("home")};window.deleteProduct=e=>{confirm("Are you sure you want to remove this product?")&&(t.products=t.products.filter(a=>a.id!==e),w(),g(),c())};window.viewOrderDetails=e=>{const a=t.orders.find(d=>d.orderId===e);if(!a){c();return}const s=t.users.find(d=>d.id===a.userId),n=s?s.name:`User ID: ${a.userId}`,i=document.createElement("div");i.className="order-details-modal",i.innerHTML=`
    ...
    `,document.body.appendChild(i);const o=d=>{d.key==="Escape"&&(i.remove(),document.removeEventListener("keydown",o))};document.addEventListener("keydown",o)};const g=()=>{const e=document.getElementById("app");let a="";switch(t.route){case"home":a=I();break;case"products":a=T();break;case"product-detail":a=B();break;case"login":a=N();break;case"signup":a=A();break;case"cart":a=j();break;case"checkout":a=q();break;case"order-confirmation":a=U();break;case"admin":a=Q();break;default:a=I()}e.innerHTML=`
        ${Header()}
        <main>
            ${a}
        </main>
    ...
    `}).join(""),a){const o=t.cart.reduce((u,p)=>u+(p.selected!==!1?p.price*p.quantity:0),0),d=t.cart.filter(u=>u.selected!==!1).length;a.innerHTML=`
        ...
        `)}const z=async()=>{await f.getProducts(),t.currentUser?.role==="admin"&&await Promise.all([f.getOrders(),f.getUsers()]),g()};z();
