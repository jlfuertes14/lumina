(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))r(a);new MutationObserver(a=>{for(const n of a)if(n.type==="childList")for(const l of n.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&r(l)}).observe(document,{childList:!0,subtree:!0});function i(a){const n={};return a.integrity&&(n.integrity=a.integrity),a.referrerPolicy&&(n.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?n.credentials="include":a.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function r(a){if(a.ep)return;a.ep=!0;const n=i(a);fetch(a.href,n)}})();const le="http://localhost:3000",ce="https://lumina-jlfuertes14s-projects.vercel.app";function me(e){const t=e.trim();return t?t.replace(/\/api\/?$/i,"").replace(/\/+$/,""):""}const te=me("https://lumina-backend-jr2a.onrender.com"),pe=te||ce,N=window.location.hostname,oe=N==="localhost"||N==="127.0.0.1",ie=N.endsWith(".onrender.com"),re=N.endsWith(".vercel.app"),ae=ie||re,ne=oe?le:ae?window.location.origin:pe,V=ae?"/api":`${ne}/api`,ue=oe?"DEVELOPMENT":ie?"RENDER":re?"VERCEL":"PRODUCTION",ge=te?"VITE_BACKEND_ORIGIN":"DEFAULT_PRODUCTION_BACKEND_ORIGIN";async function $(e,t={}){const i=`${V}${e}`;try{const r=await fetch(i,{...t,headers:{"Content-Type":"application/json",...t.headers}}),a=await r.json();if(!r.ok)throw new Error(a.error||a.message||"API request failed");return a}catch(r){throw console.error("API Error:",r),r}}console.log(`🌍 Environment: ${ue}`);console.log(`🧭 Backend origin (${ge}): ${ne}`);console.log(`🔗 API URL: ${V}`);const ve=({Breadcrumbs:e})=>{const t=[{name:"Marco Dela Cruz",role:"Founder & CEO",image:"https://ui-avatars.com/api/?name=Marco+Dela+Cruz&background=6366f1&color=fff&size=120"},{name:"Rina Gonzales",role:"Operations Manager",image:"https://ui-avatars.com/api/?name=Rina+Gonzales&background=6366f1&color=fff&size=120"},{name:"Luis Navarro",role:"Technical Support Engineer",image:"https://ui-avatars.com/api/?name=Luis+Navarro&background=6366f1&color=fff&size=120"},{name:"Ella Ramirez",role:"Customer Service Lead",image:"https://ui-avatars.com/api/?name=Ella+Ramirez&background=6366f1&color=fff&size=120"}];return`
    ...
    `,y=e=>new Intl.NumberFormat("en-PH",{style:"currency",currency:"PHP"}).format(e),we=({Breadcrumbs:e,state:t})=>{const i=t.products.slice(0,8).map((r,a)=>({...r,originalPrice:r.price*1.3,discount:[10,15,20,25,30][a%5],category:["Microcontrollers","Sensors","Tools","Robotics","Components"][a%5]}));return t.myCoupons,`
        <div style="background: #f8f9fa; min-height: 100vh; padding: 2rem 0;">
            <!-- Content Container -->
            <div style="max-width: 1400px; margin: 0 auto; padding: 0 2rem;">
                ${e("deals")}
    ...
    `,c=()=>{const{productSearch:x,productCategoryFilter:p,productSortBy:w,productSortOrder:A}=window.adminState,O=[...new Set(e.products.map(k=>k.category))].sort(),D=k=>{const L={Microcontrollers:"MIC",Sensors:"SEN",Components:"COM",Robotics:"ROB","Cables & Wires":"CAB","3D Printing":"3DP",Power:"PWR",Displays:"DIS",Modules:"MOD"}[k.category]||k.category?.substring(0,3).toUpperCase()||"GEN",R=(k._id||k.id||"").toString().slice(-4).toUpperCase();return`LUM-${L}-${R}`};let B=e.products.filter(k=>{const z=x.toLowerCase(),L=D(k).toLowerCase(),R=x===""||k.name.toLowerCase().includes(z)||(k._id||k.id).toString().toLowerCase().includes(z)||L.includes(z),de=p==="all"||k.category===p;return R&&de});w&&(B=[...B].sort((k,z)=>{let L=0;switch(w){case"price":L=k.price-z.price;break;case"stock":L=k.stock-z.stock;break;case"status":L=(z.stock>0?1:0)-(k.stock>0?1:0);break}return A==="asc"?L:-L}));const F=k=>w!==k?"⇅":A==="asc"?"↑":"↓",q=k=>w===k?"color: #3b82f6; font-weight: 600;":"";return`
        <div class="admin-page-header">
    ...
    `},xe=e=>{const t=e.stock<10,i=t?"low-stock":"",r=t?"Low Stock":"In Stock";return`
    ...
    `},ke=({Breadcrumbs:e,state:t})=>{const i=t.products.find(l=>l.id===t.currentProductId);if(!i)return navigate("home"),"";i.stock<10;const r=i.stock>0?"In Stock":"Out of Stock",a=i.stock>0?"var(--success)":"var(--danger)",n=t.products.filter(l=>l.id!==i.id).sort(()=>.5-Math.random()).slice(0,4);return`
    ...
    `},G=e=>{const t=e.stock<10,i=t?"low-stock":"",r=t?"Low Stock":"In Stock";return`
        <div class="product-card" onclick="window.viewProduct(${e.id})" style="cursor: pointer;">
            <div class="product-badge ${i}">${r}</div>
            <div class="product-image">
                <img src="${e.image}" alt="${e.name}" />
    ...
    `},K=()=>{if(o.searchQuery){const i=o.products.filter(r=>r.name.toLowerCase().includes(o.searchQuery.toLowerCase())||r.category.toLowerCase().includes(o.searchQuery.toLowerCase())||r.description.toLowerCase().includes(o.searchQuery.toLowerCase()));return`
        ...
        <div style="padding: 2rem 0;">
            ...
            <div class="product-grid">
                ${o.products.filter(i=>e.includes(i.id)).sort((i,r)=>e.indexOf(i.id)-e.indexOf(r.id)).map(G).join("")}
            </div>
        </div>
    `},Se=()=>`
    ...
    `},Ie=()=>{if(!o.currentUser)return P("login"),"";const e=o.devices||[];return`
    ...
    `:(P("login"),""),Me=()=>{if(!o.currentUser)return P("login"),"";if(!o.currentDeviceId)return P("my-devices"),"";const e=o.devices?.find(n=>n.deviceId===o.currentDeviceId);if(!e)return P("my-devices"),"";const t=o.deviceStatus[e.deviceId]||{},i=o.telemetryData[e.deviceId]||{},r=t.status==="online",a=i.obstacleAvoidance||!1;return i.speed,`
    ...
    `},Ae=()=>{if(!o.lastOrderId)return P("home"),"";const e=o.orders.find(n=>n.orderId===o.lastOrderId);if(!e)return P("home"),"";const t=new Date,i=new Date(t);i.setDate(t.getDate()+3);const r=new Date(t);r.setDate(t.getDate()+5);const a=`${i.toLocaleDateString("en-US",{month:"short",day:"numeric"})} - ${r.toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"})} `;return`
    ...
    `;document.body.insertAdjacentHTML("beforeend",r)};window.closeModal=()=>{const e=document.getElementById("orderModal");e&&e.remove()};const Le=(e=null)=>{const t=e?o.products.find(r=>r.id===e):null,i=!!t;return`
    ...
    `,a=window.open("","_blank","width=300,height=600");a.document.write(r),a.document.close(),a.onload=function(){setTimeout(()=>{a.print()},250)}};window.handleSearchInput=e=>{const t=e.target.value;if(o.searchQuery=t,t.trim()){const i=new Set;o.products.forEach(r=>{const a=r.name.toLowerCase(),n=r.category.toLowerCase(),l=t.toLowerCase();a.includes(l)&&i.add(r.name),n.includes(l)&&i.add(r.category),a.split(" ").forEach(d=>{d.toLowerCase().startsWith(l)&&d.length>2&&i.add(d.charAt(0).toUpperCase()+d.slice(1))})}),o.searchSuggestions=Array.from(i).slice(0,8),o.showSuggestions=!0}else if(o.searchSuggestions=[],o.showSuggestions=!1,o.route==="home"||o.route==="products"){C();return}se()};function se(){const e=document.querySelector(".search-container");if(!e)return;const t=e.querySelector(".search-suggestions");if(t&&t.remove(),o.showSuggestions&&o.searchQuery){const i=`
    ...
    `,document.body.appendChild(a);const n=l=>{l.key==="Escape"&&(a.remove(),document.removeEventListener("keydown",n))};document.addEventListener("keydown",n)};const C=async()=>{const e=document.getElementById("app");let t="";switch(o.route){case"home":t=K();break;case"products":t=be({Breadcrumbs:E,state:o});break;case"product-detail":t=ke({Breadcrumbs:E,state:o});break;case"signup":t=Se();break;case"cart":t=Pe();break;case"checkout":t=Te();break;case"order-confirmation":t=Ae();break;case"contact-us":t=ye({Breadcrumbs:E});break;case"about-us":t=ve({Breadcrumbs:E});break;case"learn":t=he({Breadcrumbs:E,state:o});break;case"tutorial-detail":t=fe({Breadcrumbs:E,state:o});break;case"deals":t=we({Breadcrumbs:E,state:o});break;case"admin":t=J(o);break;case"user":t=Y({state:o});break;case"admin-profile":t=_({state:o});break;case"my-devices":t=Ie();break;case"device-pair":t=De();break;case"remote-control":t=Me();break;default:t=K()}if(o.route==="admin"){e.innerHTML=J(o),window.initAdminCharts&&setTimeout(()=>window.initAdminCharts(),100);return}if(o.route==="admin-profile"){e.innerHTML=H()+_({state:o});return}if(o.route==="user"){o.currentUser&&o.orders.length===0&&await M.getMyOrders(),o.currentUser&&!o.myCoupons&&await M.getMyCoupons(),e.innerHTML=H()+Y({state:o});return}e.innerHTML=`
        ${H()}
        <main>
            ${t}
        </main>
