var Q=(e,t)=>()=>(t||e((t={exports:{}}).exports,t),t.exports);var ue=Q((he,M)=>{(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))r(a);new MutationObserver(a=>{for(const n of a)if(n.type==="childList")for(const d of n.addedNodes)d.tagName==="LINK"&&d.rel==="modulepreload"&&r(d)}).observe(document,{childList:!0,subtree:!0});function o(a){const n={};return a.integrity&&(n.integrity=a.integrity),a.referrerPolicy&&(n.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?n.credentials="include":a.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function r(a){if(a.ep)return;a.ep=!0;const n=o(a);fetch(a.href,n)}})();let q=class{constructor(t="https://lumina-production-a4bb.up.railway.app"){this.serverUrl=t,this.socket=null,this.connected=!1,this.currentDeviceId=null,this.eventHandlers=new Map}async connect(t,o=""){if(this.socket&&this.connected){console.warn("Already connected to WebSocket");return}return new Promise((r,a)=>{if(typeof io>"u"){const n=document.createElement("script");n.src="https://cdn.socket.io/4.5.4/socket.io.min.js",n.onload=()=>this._initializeSocket(t,o,r,a),n.onerror=()=>a(new Error("Failed to load Socket.IO")),document.head.appendChild(n)}else this._initializeSocket(t,o,r,a)})}_initializeSocket(t,o,r,a){try{this.socket=io(`${this.serverUrl}/control`,{auth:{userId:t,sessionToken:o},transports:["websocket","polling"]}),this.socket.on("connect",()=>{console.log("✅ Connected to ESP32 WebSocket server"),this.connected=!0,this._triggerEvent("connected"),r()}),this.socket.on("connect_error",n=>{console.error("❌ Connection error:",n.message),this.connected=!1,this._triggerEvent("error",n),a(n)}),this.socket.on("disconnect",()=>{console.log("❌ Disconnected from WebSocket server"),this.connected=!1,this._triggerEvent("disconnected")}),this.socket.on("device:status",n=>{console.log("📊 Device status:",n),this._triggerEvent("device:status",n)}),this.socket.on("device:telemetry",n=>{this._triggerEvent("device:telemetry",n)}),this.socket.on("command:response",n=>{this._triggerEvent("command:response",n)}),this.socket.on("command:sent",n=>{this._triggerEvent("command:sent",n)}),this.socket.on("device:error",n=>{console.error("Device error:",n),this._triggerEvent("device:error",n)}),this.socket.on("devices:list",n=>{this._triggerEvent("devices:list",n)}),this.socket.on("error",n=>{console.error("System error:",n),this._triggerEvent("error",n)})}catch(n){a(n)}}monitorDevice(t){if(!this.connected)throw new Error("Not connected to WebSocket server");this.currentDeviceId=t,this.socket.emit("monitor:device",t),console.log(`👁️ Monitoring device: ${t}`)}stopMonitoring(){this.currentDeviceId&&this.connected&&(this.socket.emit("monitor:stop",this.currentDeviceId),this.currentDeviceId=null)}sendCommand(t,o,r={}){if(!this.connected)throw new Error("Not connected to WebSocket server");this.socket.emit("control:command",{deviceId:t,command:o,payload:r}),console.log(`📤 Sent command to ${t}:`,o,r)}move(t,o,r=255){this.sendCommand(t,"move",{direction:o,speed:r})}stop(t){this.sendCommand(t,"stop")}turnLeft(t,o=200){this.sendCommand(t,"move",{direction:"left",speed:o})}turnRight(t,o=200){this.sendCommand(t,"move",{direction:"right",speed:o})}forward(t,o=255){this.sendCommand(t,"move",{direction:"forward",speed:o})}backward(t,o=255){this.sendCommand(t,"move",{direction:"backward",speed:o})}requestDeviceList(){if(!this.connected)throw new Error("Not connected to WebSocket server");this.socket.emit("devices:list")}on(t,o){this.eventHandlers.has(t)||this.eventHandlers.set(t,[]),this.eventHandlers.get(t).push(o)}off(t,o){if(!this.eventHandlers.has(t))return;const r=this.eventHandlers.get(t),a=r.indexOf(o);a>-1&&r.splice(a,1)}_triggerEvent(t,o=null){if(!this.eventHandlers.has(t))return;this.eventHandlers.get(t).forEach(a=>{try{a(o)}catch(n){console.error(`Error in event handler for ${t}:`,n)}})}disconnect(){this.socket&&(this.stopMonitoring(),this.socket.disconnect(),this.socket=null,this.connected=!1,console.log("👋 Disconnected from WebSocket"))}isConnected(){return this.connected}getCurrentDevice(){return this.currentDeviceId}};typeof M<"u"&&M.exports&&(M.exports=q);typeof window<"u"&&(window.ESP32SocketClient=q);const W="https://lumina-production-a4bb.up.railway.app",G="http://localhost:3000",F=window.location.hostname==="jlfuertes14.github.io",z=F?`${W}/api`:`${G}/api`;async function C(e,t={}){const o=`${z}${e}`;try{const r=await fetch(o,{...t,headers:{"Content-Type":"application/json",...t.headers}}),a=await r.json();if(!r.ok)throw new Error(a.error||a.message||"API request failed");return a}catch(r){throw console.error("API Error:",r),r}}console.log(`🌍 Environment: ${F?"PRODUCTION":"DEVELOPMENT"}`);console.log(`🔗 API URL: ${z}`);const V=({Breadcrumbs:e})=>{const t=[{name:"Marco Dela Cruz",role:"Founder & CEO",image:"https://ui-avatars.com/api/?name=Marco+Dela+Cruz&background=6366f1&color=fff&size=120"},{name:"Rina Gonzales",role:"Operations Manager",image:"https://ui-avatars.com/api/?name=Rina+Gonzales&background=6366f1&color=fff&size=120"},{name:"Luis Navarro",role:"Technical Support Engineer",image:"https://ui-avatars.com/api/?name=Luis+Navarro&background=6366f1&color=fff&size=120"},{name:"Ella Ramirez",role:"Customer Service Lead",image:"https://ui-avatars.com/api/?name=Ella+Ramirez&background=6366f1&color=fff&size=120"}];return`
    ...
    `,u=e=>new Intl.NumberFormat("en-PH",{style:"currency",currency:"PHP"}).format(e),X=({Breadcrumbs:e,state:t})=>{const o=t.products.slice(0,8).map((r,a)=>({...r,originalPrice:r.price*1.3,discount:[10,15,20,25,30][a%5],category:["Microcontrollers","Sensors","Tools","Robotics","Components"][a%5]}));return t.myCoupons,`
        <div style="background: #f8f9fa; min-height: 100vh; padding: 2rem 0;">
            <!-- Content Container -->
            <div style="max-width: 1400px; margin: 0 auto; padding: 0 2rem;">
                ${e("deals")}
    ...
    `,c=()=>`
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card">
                <div class="stat-header">
                    <div><div class="stat-title">Active Customers</div><div class="stat-value">${g}</div></div>
                ...
                <tbody>
                    ${h.map(v=>`
                        <tr>
                            ...
                            <td>${e.users.find(k=>k.id===v.userId)?.name||"Unknown"}</td>
                            <td>${u(v.total)}</td>
                ...
                <tbody>
                    ${e.products.map(v=>`
                        <tr>
                            ...
                            <td>
                                ${r?`<button class="btn-ghost" onclick="window.toggleAdminModal(true, 'editProduct', '${v.id}')">✏️</button>`:""}
                                ${n?`<button class="btn-ghost" style="color: var(--danger);" onclick="if(confirm('Delete?')) window.showToast('Deleted')">🗑️</button>`:""}
                            </td>
                        </tr>
                    `).join("")}
                </tbody>
    ...
    `,f=()=>{const v=e.users?e.users.filter(k=>["admin","staff","inventory_manager","manager"].includes(k.role)):[];return`
        <div class="admin-page-header">
        ...
        <div class="admin-table-container">
            <table class="admin-table">
                ...
                <tbody>
                    ${v.length>0?v.map(k=>`
                        <tr>
                            <td><div style="font-weight: 500;">${k.name}</div></td>
                            <td><span class="status-badge status-${k.role==="admin"?"admin":"staff"}">${k.role.toUpperCase()}</span></td>
                            <td>${k.email}</td>
                            <td>
                                <button class="btn-ghost">✏️</button>
                                ${k.role!=="admin"?'<button class="btn-ghost" style="color: var(--danger);">🗑️</button>':""}
                            </td>
                        </tr>
    ...
    `},x=()=>{if(!window.adminState.isModalOpen)return"";let v="",k="";return window.adminState.modalType==="addProduct"?(v="Add New Product",k=`
            ...
            `):window.adminState.modalType==="addStaff"&&(v="Add Staff Member",k=`
    ...
    `};window.initAdminCharts=()=>{const e=document.getElementById("salesChart");e&&(window.salesChartInstance&&window.salesChartInstance.destroy(),window.salesChartInstance=new Chart(e,{type:"line",data:{labels:["Jan","Feb","Mar","Apr","May","Jun","Jul"],datasets:[{label:"Sales",data:[4e3,3e3,5e3,2780,6890,2390,3490],borderColor:"#3b82f6",tension:.4,fill:!0,backgroundColor:"rgba(59, 130, 246, 0.1)"}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{display:!1}},scales:{y:{beginAtZero:!0,grid:{borderDash:[2,4]}},x:{grid:{display:!1}}}}}));const t=document.getElementById("demographicsChart");t&&(window.demoChartInstance&&window.demoChartInstance.destroy(),window.demoChartInstance=new Chart(t,{type:"doughnut",data:{labels:["Male","Female","Other"],datasets:[{data:[400,300,100],backgroundColor:["#0F172A","#F97316","#CBD5E1"],borderWidth:0}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{position:"bottom"}}}}))};const O=({state:e})=>{if(!e.currentUser)return navigate("login"),"";const t=e.orders.filter(l=>l.userId===e.currentUser.id);t.filter(l=>l.status==="Pending"||l.status==="Processing"),t.filter(l=>l.status==="Shipped"),t.filter(l=>l.status==="Delivered"||l.status==="Completed");const o=e.myCoupons||[];e.params&&e.params.tab&&window.switchUserTab&&(!window._lastSwitchedTab||window._lastSwitchedTab!==e.params.tab)&&(window._lastSwitchedTab=e.params.tab,setTimeout(()=>window.switchUserTab(e.params.tab),50)),window.handleProfileUpdate||(window.handleProfileUpdate=async l=>{l.preventDefault();const c=l.target,w=c.name.value.trim(),f=c.email.value.trim(),x=c.phone.value.trim(),v=c.gender.value,k=`${c.birthYear.value}-${c.birthMonth.value}-${c.birthDay.value}`;if(!w||!f||!x||!v||!c.birthYear.value||!c.birthMonth.value||!c.birthDay.value){showToast("Please fill in all fields.");return}if(/\d/.test(w)){showToast("Name should not contain numbers.");return}if(!f.includes("@")){showToast("Please enter a valid email address.");return}if(!/^09\d{9}$/.test(x)){showToast("Phone number must be 11 digits and start with 09.");return}try{await api.updateProfile(e.currentUser.id,{name:w,email:f,phone:x,gender:v,birthDate:k}),window.render()}catch(H){console.error("Profile update failed:",H)}}),window.handleImageUpload||(window.handleImageUpload=l=>{const c=l.target.files[0];if(!c)return;if(c.size>3*1024*1024){showToast("File size exceeds 3MB limit.");return}if(!["image/jpeg","image/png"].includes(c.type)){showToast("Only .JPEG and .PNG files are allowed.");return}const w=new FileReader;w.onload=f=>{const x={...e.currentUser,avatar:f.target.result};e.currentUser=x,localStorage.setItem("currentUser",JSON.stringify(x)),showToast("Profile image updated!"),render()},w.readAsDataURL(c)}),window.handleAddressUpdate||(window.handleAddressUpdate=async l=>{l.preventDefault();const c=l.target,w={fullName:c.fullName.value,phone:c.phone.value,region:c.region.value,province:c.province.value,city:c.city.value,barangay:c.barangay.value,postalCode:c.postalCode.value,street:c.street.value,details:c.details.value};if(Object.values(w).some(f=>!f)){showToast("Please fill in all address fields.");return}try{await api.updateAddress(e.currentUser.id,w),render()}catch(f){console.error("Address update failed:",f)}}),window.handlePasswordUpdate||(window.handlePasswordUpdate=async l=>{l.preventDefault();const c=l.target,w=c.currentPassword.value,f=c.newPassword.value,x=c.confirmPassword.value;if(f.length<6){showToast("New password must be at least 6 characters.");return}if(f!==x){showToast("New passwords do not match.");return}try{await api.changePassword(e.currentUser.id,w,f),c.reset()}catch(v){console.error("Password change failed:",v)}}),window.switchUserTab||(window.switchUserTab=l=>{document.querySelectorAll(".user-tab-content").forEach(f=>f.style.display="none");const c=document.getElementById(`user-tab-${l}`);c&&(c.style.display="block"),document.querySelectorAll(".sidebar-link").forEach(f=>f.classList.remove("active"));const w=document.querySelector(`[data-tab="${l}"]`);w&&w.classList.add("active"),window.scrollTo(0,0)}),window.switchOrderTab||(window.switchOrderTab=l=>{document.querySelectorAll(".order-status-tab").forEach(c=>c.classList.remove("active")),document.querySelector(`.order-status-tab[data-status="${l}"]`).classList.add("active"),document.querySelectorAll(".order-item").forEach(c=>{l==="All"||c.dataset.status===l?c.style.display="block":c.style.display="none"})});const r=(l,c,w)=>{let f="";for(let x=l;x<=c;x++)f+=`<option value="${x}" ${parseInt(w)===x?"selected":""}>${x}</option>`;return f},a=["January","February","March","April","May","June","July","August","September","October","November","December"],n=e.currentUser.birthDate?new Date(e.currentUser.birthDate):null,d=n?n.getDate():"",m=n?n.getMonth()+1:"",s=n?n.getFullYear():"",g=e.currentUser.address||{},h=typeof g=="object"&&g!==null;return`
                        ...
                        <div id="user-tab-address" class="user-tab-content" style="display: none;">
                            ...
                            <form onsubmit="window.handleAddressUpdate(event)" style="max-width: 600px;">
                                <div class="form-group" style="margin-bottom: 1rem;">
                                    <label class="form-label">Full Name</label>
                                    <input type="text" name="fullName" class="form-input" value="${h&&g.fullName||""}" required>
                                </div>
                                <div class="form-group" style="margin-bottom: 1rem;">
                                    <label class="form-label">Phone Number</label>
                                    <input type="text" name="phone" class="form-input" value="${h&&g.phone||""}" placeholder="09XXXXXXXXX" required>
                                </div>
                                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 1rem;">
                                    <div class="form-group">
                                        <label class="form-label">Region</label>
                                        <input type="text" name="region" class="form-input" value="${h&&g.region||""}" required>
                                    </div>
                                    <div class="form-group">
                                        <label class="form-label">Province</label>
                                        <input type="text" name="province" class="form-input" value="${h&&g.province||""}" required>
                                    </div>
                                </div>
                                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 1rem;">
                                    <div class="form-group">
                                        <label class="form-label">City</label>
                                        <input type="text" name="city" class="form-input" value="${h&&g.city||""}" required>
                                    </div>
                                    <div class="form-group">
                                        <label class="form-label">Barangay</label>
                                        <input type="text" name="barangay" class="form-input" value="${h&&g.barangay||""}" required>
                                    </div>
                                </div>
                                <div class="form-group" style="margin-bottom: 1rem;">
                                    <label class="form-label">Postal Code</label>
                                    <input type="text" name="postalCode" class="form-input" value="${h&&g.postalCode||""}" required>
                                </div>
                                <div class="form-group" style="margin-bottom: 1rem;">
                                    <label class="form-label">Street Name, Building, House No.</label>
                                    <input type="text" name="street" class="form-input" value="${h&&g.street||""}" required>
                                </div>
                                <div class="form-group" style="margin-bottom: 1.5rem;">
                                    <label class="form-label">Additional Details (Landmark, etc.)</label>
                                    <input type="text" name="details" class="form-input" value="${h&&g.details||""}">
                                </div>
    ...
    `},ee=({Breadcrumbs:e,state:t})=>{const o=t.products.find(d=>d.id===t.currentProductId);if(!o)return navigate("home"),"";o.stock<10;const r=o.stock>0?"In Stock":"Out of Stock",a=o.stock>0?"var(--success)":"var(--danger)",n=t.products.filter(d=>d.id!==o.id).sort(()=>.5-Math.random()).slice(0,4);return`
    ...
    `},E=e=>{const t=e.stock<10,o=t?"low-stock":"",r=t?"Low Stock":"In Stock";return`
        <div class="product-card" onclick="window.viewProduct(${e.id})" style="cursor: pointer;">
            <div class="product-badge ${o}">${r}</div>
            <div class="product-image">
                <img src="${e.image}" alt="${e.name}" />
    ...
    `):(navigate("login"),""),i={products:[],users:[],orders:[],currentUser:JSON.parse(localStorage.getItem("currentUser"))||null,cart:JSON.parse(localStorage.getItem("cart_v2"))||[],route:sessionStorage.getItem("currentRoute")||"home",mobileMenuOpen:!1,searchQuery:"",showSuggestions:!1,searchSuggestions:[],currentProductId:null,devices:[],currentDeviceId:null,esp32Client:null,deviceStatus:{},telemetryData:{},sortBy:"featured",filterCategory:null,cartSearchQuery:"",isLoading:!1,cartSynced:!1,myCoupons:null,checkoutData:{shipping:{fullName:"",address:"",city:"",province:"",postalCode:"",phone:"",instructions:""},paymentMethod:"cod",shippingFee:50},lastOrderId:null},$={getProducts:async()=>{try{const e=await C("/products");i.products=e.data,y()}catch(e){console.error("Failed to load products:",e),p("Failed to load products")}},login:async(e,t)=>{console.log("Calling API login...");try{const o=await C("/users/login",{method:"POST",body:JSON.stringify({email:e,password:t})});i.currentUser=o.data,localStorage.setItem("currentUser",JSON.stringify(i.currentUser)),await I.getMyDevices(),await ie(),await $.getMyOrders(),p(`Welcome back, ${i.currentUser.name}!`),b("home")}catch(o){throw p(o.message||"Login failed"),o}},register:async(e,t,o)=>{try{const r=await C("/users/register",{method:"POST",body:JSON.stringify({name:e,email:t,password:o})});i.currentUser=r.data,localStorage.setItem("currentUser",JSON.stringify(i.currentUser)),p("Account created successfully!"),b("home")}catch(r){throw p(r.message||"Registration failed"),r}},createOrder:async e=>{try{const t=await C("/orders",{method:"POST",body:JSON.stringify(e)});return i.cart=[],S(),p("Order placed successfully!"),i.currentUser.role==="admin"&&$.getOrders(),t.data}catch(t){throw p(t.message||"Failed to place order"),t}},getOrders:async()=>{if(!(!i.currentUser||i.currentUser.role!=="admin"))try{const e=await C("/orders");i.orders=e.data,y()}catch(e){console.error("Failed to load orders:",e)}},getMyOrders:async()=>{if(i.currentUser)try{const e=await C(`/orders/my-orders?userId=${i.currentUser.id}`);i.orders=e.data,y()}catch(e){console.error("Failed to load orders:",e)}},getUsers:async()=>{if(!(!i.currentUser||i.currentUser.role!=="admin"))try{const e=await C("/users");i.users=e.data,y()}catch(e){console.error("Failed to load users:",e)}},updateProfile:async(e,t)=>{try{const o=await C(`/users/${e}/profile`,{method:"PUT",body:JSON.stringify(t)});return i.currentUser=o.data,localStorage.setItem("currentUser",JSON.stringify(o.data)),window.showToast("Profile updated successfully!"),o.data}catch(o){throw window.showToast(o.message||"Failed to update profile"),o}},updateAddress:async(e,t)=>{try{const o=await C(`/users/${e}/address`,{method:"PUT",body:JSON.stringify(t)});return i.currentUser=o.data,localStorage.setItem("currentUser",JSON.stringify(o.data)),window.showToast("Address saved successfully!"),o.data}catch(o){throw window.showToast(o.message||"Failed to save address"),o}},changePassword:async(e,t,o)=>{try{await C(`/users/${e}/password`,{method:"PUT",body:JSON.stringify({currentPassword:t,newPassword:o})}),window.showToast("Password changed successfully!")}catch(r){throw window.showToast(r.message||"Failed to change password"),r}},claimCoupon:async e=>{if(!i.currentUser){p("Please login to claim coupons"),window.openLoginModal();return}try{const t=await C("/coupons/claim",{method:"POST",body:JSON.stringify({userId:i.currentUser.id,couponCode:e})});return p("Coupon claimed successfully! 🎉"),t.data}catch(t){throw p(t.message||"Failed to claim coupon"),t}},getMyCoupons:async()=>{if(i.currentUser)try{const e=await C(`/coupons/my-coupons?userId=${i.currentUser.id}`);i.myCoupons=e.data,y()}catch(e){console.error("Failed to load coupons:",e)}}};window.api=$;const I={getMyDevices:async()=>{if(i.currentUser)try{const e=await C(`/devices/my-devices?userId=${i.currentUser.id}`);i.devices=e.data,te(),y()}catch(e){console.error("Failed to load devices:",e),p("Failed to load devices")}},pairDevice:async(e,t)=>{try{const o=await C("/devices/pair",{method:"POST",body:JSON.stringify({deviceId:e,deviceToken:t,userId:i.currentUser.id})});p("Device paired successfully!"),await I.getMyDevices(),b("my-devices")}catch(o){throw p(o.message||"Device pairing failed"),o}}};function te(){i.esp32Client||(i.esp32Client=new ESP32SocketClient),!i.esp32Client.isConnected()&&i.currentUser&&i.esp32Client.connect(i.currentUser.id).catch(e=>{console.error("Failed to connect to WebSocket:",e)}),i.esp32Client.on("device:status",e=>{if(console.log("📊 Device status update:",e),i.devices){const t=i.devices.findIndex(o=>o.deviceId===e.deviceId);t!==-1&&(i.devices[t].status=e.status==="online"?"active":"offline",i.devices[t].lastOnline=e.timestamp,i.deviceStatus[e.deviceId]=e,i.route==="my-devices"&&y())}})}const S=()=>{localStorage.setItem("currentUser",JSON.stringify(i.currentUser)),localStorage.setItem("cart_v2",JSON.stringify(i.cart))},ie=async()=>{if(!(!i.currentUser||i.cartSynced))try{const e=await C(`/users/${i.currentUser.id}/cart/sync`,{method:"POST",body:JSON.stringify({localCart:i.cart})});i.cart=e.data.map(t=>({id:t.productId,name:t.name,price:t.price,image:t.image,quantity:t.quantity,category:t.category,selected:t.selected})),i.cartSynced=!0,S(),y(),p("Cart synced!")}catch(e){console.error("Cart sync failed:",e)}},b=(e,t={})=>{i.route=e,i.params=t,i.mobileMenuOpen=!1,sessionStorage.setItem("currentRoute",e),y(),window.scrollTo(0,0)};window.navigate=b;const p=e=>{const t=document.createElement("div");t.className="toast",t.textContent=e,document.body.appendChild(t),setTimeout(()=>t.classList.add("show"),100),setTimeout(()=>{t.classList.remove("show"),setTimeout(()=>t.remove(),300)},3e3)};window.showToast=p;const P=e=>`
        <div class="breadcrumbs">
            <a href="#" onclick="window.navigate('home'); return false;">Home</a>
    ...
    `,T=()=>{const e=i.currentUser!==null,t=i.currentUser?.role==="admin",o=i.route==="cart",r=i.cart.length;return`
        <header>
    ...
    `},U=()=>{if(i.searchQuery){const o=i.products.filter(r=>r.name.toLowerCase().includes(i.searchQuery.toLowerCase())||r.category.toLowerCase().includes(i.searchQuery.toLowerCase())||r.description.toLowerCase().includes(i.searchQuery.toLowerCase()));return`
            <div class="hero">
        ...
        <div style="padding: 2rem 0;">
            ...
            <div class="product-grid">
                ${i.products.filter(o=>e.includes(o.id)).sort((o,r)=>e.indexOf(o.id)-e.indexOf(r.id)).map(E).join("")}
            </div>
        </div>
    `},oe=()=>`
    ...
    `},ae=()=>{if(!i.currentUser)return b("login"),"";const e=i.devices||[];return`
    ...
    `},de=()=>{if(!i.currentUser)return b("login"),"";const e=i.cart.filter(a=>a.selected!==!1);if(e.length===0)return p("No items selected for checkout"),b("cart"),"";const t=e.reduce((a,n)=>a+n.price*n.quantity,0),o=i.checkoutData.shippingFee,r=t+o;return`
    ...
    `},le=()=>{if(!i.lastOrderId)return b("home"),"";const e=i.orders.find(n=>n.orderId===i.lastOrderId);if(!e)return b("home"),"";const t=new Date,o=new Date(t);o.setDate(t.getDate()+3);const r=new Date(t);r.setDate(t.getDate()+5);const a=`${o.toLocaleDateString("en-US",{month:"short",day:"numeric"})} - ${r.toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"})} `;return`
    ...
    `};window.navigateToTutorial=e=>{b("tutorial-detail",{id:e})};window.filterLearningContent=e=>{i.filterLearnCategory=e,y()};window.navigateToTutorial=e=>{b("tutorial-detail",{id:e})};window.updateOrderStatus=(e,t)=>{const o=i.orders.find(r=>r.id===e);o&&(o.status=t,S(),p(`Order #${e} updated to ${t}`),y())};window.toggleSelectAll=e=>{document.querySelectorAll(".product-checkbox").forEach(o=>o.checked=e.checked)};window.bulkAction=e=>{const t=document.querySelectorAll(".product-checkbox:checked"),o=Array.from(t).map(r=>parseInt(r.value));if(o.length===0){p("No products selected");return}e==="delete"?confirm(`Delete ${o.length} products?`)&&(i.products=i.products.filter(r=>!o.includes(r.id)),S(),y(),p("Products deleted")):e==="restock"&&(i.products.forEach(r=>{o.includes(r.id)&&(r.stock+=10)}),S(),y(),p("Products restocked"))};window.viewOrderDetails=e=>{const t=i.orders.find(a=>a.id===e);if(!t)return;const o=i.users.find(a=>a.id===t.userId),r=`
    ...
    `;document.body.insertAdjacentHTML("beforeend",r)};window.closeModal=()=>{const e=document.getElementById("orderModal");e&&e.remove()};const ce=(e=null)=>{const t=e?i.products.find(r=>r.id===e):null,o=!!t;return`
    ...
    `};window.navigate=b;window.toggleMobileMenu=()=>{i.mobileMenuOpen=!i.mobileMenuOpen,y()};window.showProductModal=(e=null)=>{document.body.insertAdjacentHTML("beforeend",ce(e))};window.closeProductModal=()=>{document.getElementById("productModal")?.remove()};window.editProduct=e=>{window.showProductModal(e)};window.deleteProduct=async e=>{if(confirm("Delete this product?"))try{await C(`/products/${e}`,{method:"DELETE"}),await $.getProducts(),p("Product deleted")}catch{p("Delete failed")}};window.handleProductSubmit=async(e,t)=>{e.preventDefault();const o=new FormData(e.target);try{const r=t?"PUT":"POST",a=t?`/products/${t}`:"/products";if(!(await fetch(`${z}${a}`,{method:r,body:o})).ok)throw new Error("Failed");await $.getProducts(),window.closeProductModal(),p(t?"Product updated!":"Product created!")}catch{p("Operation failed")}};window.handleSort=e=>{i.sortBy=e,y()};window.viewProduct=e=>{i.currentProductId=e,b("product-detail")};window.adjustDetailQty=e=>{const t=document.getElementById("detailQty");let o=parseInt(t.value)+e;o<1&&(o=1),t.value=o};window.addToCartFromDetail=e=>{const t=parseInt(document.getElementById("detailQty").value);if(!i.currentUser){p("Please login to shop"),window.openLoginModal();return}const o=i.products.find(a=>a.id===e),r=i.cart.find(a=>a.id===e);r?r.quantity+=t:i.cart.push({...o,quantity:t,selected:!0}),S(),p(`Added ${t} item(s) to cart`)};window.addToCart=async e=>{if(!i.currentUser){window.showToast("Please login to shop"),window.openLoginModal();return}const t=i.products.find(r=>r.id===e);if(!t)return;const o=i.cart.find(r=>r.id===e);if(o?o.quantity+=1:i.cart.push({...t,quantity:1,selected:!0}),S(),y(),window.showToast(`Added ${t.name} to cart! 🛒`),i.currentUser)try{await C(`/users/${i.currentUser.id}/cart`,{method:"PUT",body:JSON.stringify({cart:i.cart.map(r=>({productId:r.id,name:r.name,price:r.price,image:r.image,quantity:r.quantity,category:r.category,selected:r.selected!==!1}))})})}catch(r){console.error("Failed to sync cart:",r)}};window.updateQuantity=(e,t)=>{if(t<1){window.removeFromCart(e);return}const o=i.cart.find(r=>r.id===e);o&&(o.quantity=t,S(),y())};window.removeFromCart=e=>{i.cart=i.cart.filter(t=>t.id!==e),S(),y()};window.checkout=async()=>{if(i.cart.length===0)return;if(!i.currentUser){p("Please login to checkout"),b("login");return}if(i.cart.filter(t=>t.selected!==!1).length===0){p("No items selected for checkout");return}b("checkout")};window.updateShippingInfo=(e,t)=>{i.checkoutData.shipping[e]=t};window.selectPaymentMethod=e=>{i.checkoutData.paymentMethod=e,y()};window.handlePhoneInput=e=>{const t=e.value.replace(/[^0-9]/g,"");e.value=t,i.checkoutData.shipping.phone=t};window.handleNameInput=e=>{const t=e.value.replace(/[^a-zA-Z\s]/g,"");e.value=t,i.checkoutData.shipping.fullName=t};window.handleLocationInput=(e,t)=>{const o=e.value.replace(/[^a-zA-Z\s]/g,"");e.value=o,i.checkoutData.shipping[t]=o};window.handlePostalInput=e=>{const t=e.value.replace(/[^0-9]/g,"");e.value=t,i.checkoutData.shipping.postalCode=t};window.showPaymentModal=e=>new Promise(t=>{let o="";const r=`
        ...
        `;document.body.insertAdjacentHTML("beforeend",r);const a=document.getElementById("paymentModalOverlay"),n=document.getElementById("paymentAmountInput"),d=document.getElementById("paymentError"),m=document.getElementById("paymentChangeDisplay"),s=document.getElementById("changeAmount"),g=document.getElementById("paymentConfirmBtn"),h=document.getElementById("paymentCancelBtn");setTimeout(()=>n.focus(),100),n.addEventListener("input",l=>{let c=l.target.value.replace(/[^0-9.]/g,"");const w=c.split(".");w.length>2&&(c=w[0]+"."+w.slice(1).join("")),l.target.value=c,o=c;const f=parseFloat(c);if(!c||isNaN(f)||f<0){d.textContent="",d.classList.remove("show"),n.classList.remove("error"),g.disabled=!0,m.classList.remove("show");return}if(f<e){const x=e-f;d.textContent=`Insufficient! Need ${u(x)} more`,d.classList.add("show"),n.classList.add("error"),g.disabled=!0,m.classList.remove("show")}else{const x=f-e;d.classList.remove("show"),n.classList.remove("error"),g.disabled=!1,m.classList.add("show"),s.textContent=u(x)}}),document.querySelectorAll(".quick-amount-btn").forEach(l=>{l.addEventListener("click",()=>{const c=l.getAttribute("data-amount");n.value=c,n.dispatchEvent(new Event("input"))})}),h.addEventListener("click",()=>{a.remove(),t(null)}),a.addEventListener("click",l=>{l.target===a&&(a.remove(),t(null))}),g.addEventListener("click",()=>{const l=parseFloat(o);l>=e&&(a.remove(),t({amountPaid:l,change:l-e}))}),n.addEventListener("keypress",l=>{l.key==="Enter"&&!g.disabled&&g.click()}),document.addEventListener("keydown",function l(c){c.key==="Escape"&&(a.remove(),t(null),document.removeEventListener("keydown",l))})});window.showGCashModal=e=>new Promise(t=>{const o="GCASH-"+Date.now().toString().slice(-8),r=`
            <div class="payment-modal-overlay" id="paymentModalOverlay">
                <div class="payment-modal">
        ...
        `;document.body.insertAdjacentHTML("beforeend",r);const a=document.getElementById("paymentModalOverlay"),n=document.getElementById("paymentConfirmBtn"),d=document.getElementById("paymentCancelBtn"),m=document.getElementById("processingOverlay"),s=document.getElementById("successOverlay");d.addEventListener("click",()=>{a.remove(),t(null)}),n.addEventListener("click",()=>{m.classList.add("show"),setTimeout(()=>{m.classList.remove("show"),s.classList.add("show"),setTimeout(()=>{a.remove(),t({method:"maya",reference:o})},1e3)},1500)}),a.addEventListener("click",g=>{g.target===a&&(a.remove(),t(null))})});window.showCardModal=e=>new Promise(t=>{const d=`
            <div class="payment-modal-overlay" id="paymentModalOverlay">
                <div class="payment-modal">
                    <div class="demo-badge">DEMO MODE</div>
                    <div class="payment-modal-header">
        ...
        `;document.body.insertAdjacentHTML("beforeend",d);const m=document.getElementById("paymentModalOverlay"),s=document.getElementById("paymentConfirmBtn"),g=document.getElementById("paymentCancelBtn"),h=document.getElementById("processingOverlay"),l=document.getElementById("successOverlay");g.addEventListener("click",()=>{m.remove(),t(null)}),s.addEventListener("click",()=>{h.classList.add("show"),setTimeout(()=>{h.classList.remove("show"),l.classList.add("show"),setTimeout(()=>{m.remove(),t({method:"card",last4:"1111"})},1e3)},2e3)}),m.addEventListener("click",c=>{c.target===m&&(m.remove(),t(null))})});window.showBankModal=e=>new Promise(t=>{const o="BDO-"+Date.now().toString().slice(-8),r=`
            <div class="payment-modal-overlay" id="paymentModalOverlay">
                <div class="payment-modal">
                    <div class="demo-badge">DEMO MODE</div>
                    <div class="payment-modal-header">
        ...
        `;document.body.insertAdjacentHTML("beforeend",r);const a=document.getElementById("paymentModalOverlay"),n=document.getElementById("paymentConfirmBtn"),d=document.getElementById("paymentCancelBtn"),m=document.getElementById("processingOverlay"),s=document.getElementById("successOverlay");d.addEventListener("click",()=>{a.remove(),t(null)}),n.addEventListener("click",()=>{m.classList.add("show"),setTimeout(()=>{m.classList.remove("show"),s.classList.add("show"),setTimeout(()=>{a.remove(),t({method:"bank",reference:o})},1e3)},1500)}),a.addEventListener("click",g=>{g.target===a&&(a.remove(),t(null))})});window.placeOrder=async()=>{const e=i.checkoutData.shipping;if(!e.fullName||!e.fullName.trim()){p("Please enter your full name");return}if(!e.address||!e.address.trim()){p("Please enter your address");return}if(!e.city||!e.city.trim()){p("Please enter your city");return}if(!e.province||!e.province.trim()){p("Please enter your province");return}if(!e.phone||!e.phone.trim()){p("Please enter your phone number");return}if(!/^09\d{9}$/.test(e.phone)){p('Phone number must start with "09" and contain exactly 11 digits');return}const o=/^[a-zA-Z\s]+$/;if(!o.test(e.fullName)){p("Full Name must contain letters and spaces only");return}if(!o.test(e.city)){p("City must contain letters and spaces only");return}if(!o.test(e.province)){p("Province must contain letters and spaces only");return}if(e.postalCode&&!/^\d+$/.test(e.postalCode)){p("Postal Code must contain numbers only");return}if(!i.checkoutData.paymentMethod){p("Please select a payment method");return}const r=i.cart.filter(h=>h.selected!==!1),n=r.reduce((h,l)=>h+l.price*l.quantity,0)+i.checkoutData.shippingFee;let d=null;switch(i.checkoutData.paymentMethod){case"cod":d=await showPaymentModal(n);break;case"gcash":d=await showGCashModal(n);break;case"maya":d=await showMayaModal(n);break;case"card":d=await showCardModal(n);break;case"bank":d=await showBankModal(n);break;default:p("Please select a payment method");return}if(!d)return;let m=d.amountPaid||n,s=d.change||0;const g={userId:i.currentUser.id,items:r.map(h=>({productId:h.id,quantity:h.quantity,price:h.price,name:h.name})),total:n,shippingInfo:i.checkoutData.shipping,paymentMethod:i.checkoutData.paymentMethod,shippingFee:i.checkoutData.shippingFee,amountPaid:m,change:s};try{const h=await $.createOrder(g);i.lastOrderId=h.orderId,i.lastOrderPayment={amountPaid:m,change:s},i.orders.push({orderId:h.orderId,...h,items:r,total:n,createdAt:new Date().toISOString(),userId:i.currentUser.id,devices:h.devices||[]}),i.cart=i.cart.filter(l=>l.selected===!1),S(),b("order-confirmation")}catch{}};window.printReceipt=()=>{if(!i.lastOrderId){p("No order found to print");return}const e=i.orders.find(n=>n.orderId===i.lastOrderId);if(!e){p("Order not found");return}const t=i.lastOrderPayment||{},o=new Date,r=`
        <!DOCTYPE html>
        <html>
        <head>
            <meta charset="UTF-8">
    ...
    `,a=window.open("","_blank","width=300,height=600");a.document.write(r),a.document.close(),a.onload=function(){setTimeout(()=>{a.print()},250)}};window.handleSearchInput=e=>{const t=e.target.value;if(i.searchQuery=t,t.trim()){const o=new Set;i.products.forEach(r=>{const a=r.name.toLowerCase(),n=r.category.toLowerCase(),d=t.toLowerCase();a.includes(d)&&o.add(r.name),n.includes(d)&&o.add(r.category),a.split(" ").forEach(s=>{s.toLowerCase().startsWith(d)&&s.length>2&&o.add(s.charAt(0).toUpperCase()+s.slice(1))})}),i.searchSuggestions=Array.from(o).slice(0,8),i.showSuggestions=!0}else if(i.searchSuggestions=[],i.showSuggestions=!1,i.route==="home"||i.route==="products"){y();return}R()};function R(){const e=document.querySelector(".search-container");if(!e)return;const t=e.querySelector(".search-suggestions");if(t&&t.remove(),i.showSuggestions&&i.searchQuery){const o=`
        ...
        `;e.insertAdjacentHTML("beforeend",o)}}window.showSearchSuggestions=()=>{i.searchQuery&&(i.showSuggestions=!0,R())};window.selectSuggestion=e=>{i.searchQuery=e,i.showSuggestions=!1;const t=document.getElementById("searchInput");t&&(t.value=e),handleSearch()};window.handleSearch=()=>{i.showSuggestions=!1;const e=document.getElementById("searchInput");e&&(i.searchQuery=e.value.trim()),b("products"),setTimeout(()=>{const t=document.querySelector(".product-grid");t&&t.scrollIntoView({behavior:"smooth",block:"start"})},100)};window.clearSearch=()=>{i.searchQuery="",i.showSuggestions=!1,i.searchSuggestions=[],y()};document.addEventListener("click",e=>{if(!e.target.closest(".search-container")&&i.showSuggestions){i.showSuggestions=!1;const t=document.querySelector(".search-suggestions");t&&t.remove()}});window.handleLogin=async e=>{console.log("Login attempt started"),e.preventDefault();const t=e.target.email.value,o=e.target.password.value;console.log("Credentials:",{email:t,password:o});try{await $.login(t,o),console.log("Login successful")}catch(r){console.error("Login error:",r)}};window.toggleUserMenu=()=>{const e=document.getElementById("user-menu-dropdown");e&&(e.style.display=e.style.display==="none"?"block":"none")};document.addEventListener("click",e=>{if(!e.target.closest(".user-menu-container")){const t=document.getElementById("user-menu-dropdown");t&&(t.style.display="none")}});window.handleSignup=async e=>{console.log("Signup attempt started"),e.preventDefault();const t=e.target.name.value,o=e.target.email.value,r=e.target.password.value;try{await $.register(t,o,r)}catch(a){console.error("Signup error:",a)}};window.logout=()=>{i.currentUser=null,i.cart=[],localStorage.removeItem("currentUser"),localStorage.removeItem("cart_v2"),p("Logged out successfully"),sessionStorage.removeItem("currentRoute"),b("home")};window.handleContactSubmit=e=>{e.preventDefault();const t=e.target,o={name:t.name.value,email:t.email.value,subject:t.subject.value,message:t.message.value};console.log("Contact form submitted:",o),p("Thank you for your message! We will get back to you soon."),t.reset()};window.deleteProduct=e=>{confirm("Are you sure you want to remove this product?")&&(i.products=i.products.filter(t=>t.id!==e),S(),y(),p("Product removed"))};window.viewOrderDetails=e=>{const t=i.orders.find(d=>d.orderId===e);if(!t){p("Order not found");return}const o=i.users.find(d=>d.id===t.userId),r=o?o.name:`User ID: ${t.userId}`,a=document.createElement("div");a.className="order-details-modal",a.innerHTML=`
    ...
    `,document.body.appendChild(a);const n=d=>{d.key==="Escape"&&(a.remove(),document.removeEventListener("keydown",n))};document.addEventListener("keydown",n)};const y=async()=>{const e=document.getElementById("app");let t="";switch(i.route){case"home":t=U();break;case"products":t=K({Breadcrumbs:P,state:i});break;case"product-detail":t=ee({Breadcrumbs:P,state:i});break;case"signup":t=oe();break;case"cart":t=re();break;case"checkout":t=de();break;case"order-confirmation":t=le();break;case"contact-us":t=J({Breadcrumbs:P});break;case"about-us":t=V({Breadcrumbs:P});break;case"learn":t=Y({Breadcrumbs:P,state:i});break;case"tutorial-detail":t=_({Breadcrumbs:P,state:i});break;case"deals":t=X({Breadcrumbs:P,state:i});break;case"admin":t=A(i);break;case"user":t=O({state:i});break;case"admin-profile":t=B({state:i});break;case"my-devices":t=ae();break;case"device-pair":t=ne();break;case"remote-control":t=se();break;default:t=U()}if(i.route==="admin"){e.innerHTML=A(i),window.initAdminCharts&&setTimeout(()=>window.initAdminCharts(),100);return}if(i.route==="admin-profile"){e.innerHTML=T()+B({state:i});return}if(i.route==="user"){i.currentUser&&i.orders.length===0&&await $.getMyOrders(),i.currentUser&&!i.myCoupons&&await $.getMyCoupons(),e.innerHTML=T()+O({state:i});return}e.innerHTML=`
        ${T()}
        <main>
            ${t}
        </main>
    ...
    `,i.route==="admin"&&window.initAdminCharts&&setTimeout(()=>window.initAdminCharts(),100),i.route==="deals"&&setTimeout(()=>window.startDealsTimer(),100)};window.render=y;window.handleCartSearch=e=>{i.cartSearchQuery=e.target.value,D()};window.clearCartSearch=()=>{i.cartSearchQuery="",D()};window.toggleCartItem=e=>{const t=i.cart.find(o=>o.id===e);t&&(t.selected=t.selected===!1,S(),D())};window.toggleFindSimilar=e=>{const t=i.cart.find(o=>o.id===e);t&&(i.cart.forEach(o=>{o.id!==e&&(o.showSimilar=!1)}),t.showSimilar=!t.showSimilar,S(),D())};function D(){const e=document.querySelector(".cart-items"),t=document.querySelector(".cart-summary"),o=document.querySelector(".cart-search-message");if(!e)return;let r=i.cart;if(i.cartSearchQuery&&(r=i.cart.filter(n=>n.name.toLowerCase().includes(i.cartSearchQuery.toLowerCase())||n.category.toLowerCase().includes(i.cartSearchQuery.toLowerCase()))),e.innerHTML=r.map(n=>{const d=i.products.filter(m=>m.category===n.category&&m.id!==n.id).slice(0,4);return`
        <div style="display: flex; flex-direction: column; background: var(--surface); border-bottom: 1px solid var(--border);">
            <div class="cart-item" style="border-bottom: none;">
                <input type="checkbox" 
                    style="width: 20px; height: 20px; margin-right: 1rem; cursor: pointer; accent-color: var(--primary);"
    ...
    `),setTimeout(()=>{document.getElementById("loginModal").classList.add("show")},10)};window.closeLoginModal=()=>{const e=document.getElementById("loginModal");e&&(e.classList.remove("show"),setTimeout(()=>e.remove(),300))};window.closeLoginModalOnOverlay=e=>{e.target.id==="loginModal"&&window.closeLoginModal()};window.handleModalLogin=async e=>{e.preventDefault();const t=new FormData(e.target),o=t.get("email"),r=t.get("password");try{await $.login(o,r),window.closeLoginModal()}catch(a){console.error("Login error:",a)}};const pe=window.navigate;window.navigate=(e,t)=>{window.showLoading(),setTimeout(()=>{pe(e,t),window.hideLoading()},300)};window.handleDevicePairing=async e=>{e.preventDefault();const t=e.target,o=t.deviceId.value.trim(),r=t.deviceToken.value.trim();try{await I.pairDevice(o,r)}catch{}};window.handleDeviceUnpairing=async e=>{try{await I.unpairDevice(e)}catch{}};window.startRemoteControl=async e=>{i.currentDeviceId=e,i.esp32Client||(i.esp32Client=new ESP32SocketClient);try{await i.esp32Client.connect(i.currentUser.id),i.esp32Client.monitorDevice(e),i.esp32Client.on("device:status",t=>{i.deviceStatus[t.deviceId]=t,j()}),i.esp32Client.on("device:telemetry",t=>{i.telemetryData[t.deviceId]=t,j()}),i.esp32Client.on("command:sent",t=>{L(`Command sent: ${t.command}`)}),i.esp32Client.on("command:response",t=>{t.success?L(`✓ ${t.message||"Command executed"}`):L(`✗ ${t.error||"Command failed"}`)}),b("remote-control")}catch(t){p("Failed to connect to device"),console.error(t)}};window.stopRemoteControl=()=>{i.esp32Client&&i.esp32Client.stopMonitoring(),i.currentDeviceId=null,b("my-devices")};window.sendCarCommand=e=>{if(!(!i.esp32Client||!i.currentDeviceId))try{e==="stop"?i.esp32Client.stop(i.currentDeviceId):i.esp32Client.move(i.currentDeviceId,e,255)}catch(t){p("Failed to send command"),console.error(t)}};window.toggleLights=()=>{if(!i.esp32Client||!i.currentDeviceId)return;const e=document.getElementById("lightsToggle");if(e){e.classList.toggle("active");const t=e.classList.contains("active");try{i.esp32Client.sendCommand(i.currentDeviceId,"lights",{state:t?"on":"off"}),p(`Lights ${t?"ON":"OFF"}`)}catch(o){console.error("Failed to toggle lights:",o),e.classList.toggle("active")}}};window.toggleHorn=()=>{if(!i.esp32Client||!i.currentDeviceId)return;const e=document.getElementById("hornToggle");if(e){e.classList.toggle("active");const t=e.classList.contains("active");try{i.esp32Client.sendCommand(i.currentDeviceId,"horn",{state:t?"on":"off"}),p(`Horn ${t?"ON":"OFF"}`)}catch(o){console.error("Failed to toggle horn:",o),e.classList.toggle("active")}}};function j(){i.route==="remote-control"&&y()}function L(e){const t=document.getElementById("commandLogContent");if(t){const o=document.createElement("div");for(o.className="log-entry",o.textContent=`[${new Date().toLocaleTimeString()}] ${e}`,t.insertBefore(o,t.firstChild);t.children.length>10;)t.removeChild(t.lastChild)}}window.toggleToken=e=>{i.showTokens||(i.showTokens={}),i.showTokens[e]=!i.showTokens[e],y()};window.handleCategoryFilter=e=>{i.filterCategory=e||null,y()};document.addEventListener("keydown",e=>{if(i.route!=="remote-control")return;const t=e.key.toLowerCase(),o={w:"forward",arrowup:"forward",s:"backward",arrowdown:"backward",a:"left",arrowleft:"left",d:"right",arrowright:"right"," ":"stop"};o[t]&&(e.preventDefault(),window.sendCarCommand(o[t]))});document.addEventListener("keydown",e=>{e.key==="Escape"&&document.getElementById("loginModal")&&window.closeLoginModal()});document.addEventListener("keyup",e=>{if(i.route!=="remote-control")return;const t=e.key.toLowerCase();["w","s","a","d","arrowup","arrowdown","arrowleft","arrowright"].includes(t)&&(e.preventDefault(),window.sendCarCommand("stop"))});me()});export default ue();
