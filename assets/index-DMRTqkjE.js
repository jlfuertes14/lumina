var j=(t,e)=>()=>(e||t((e={exports:{}}).exports,e),e.exports);var se=j((le,$)=>{(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))r(n);new MutationObserver(n=>{for(const s of n)if(s.type==="childList")for(const d of s.addedNodes)d.tagName==="LINK"&&d.rel==="modulepreload"&&r(d)}).observe(document,{childList:!0,subtree:!0});function o(n){const s={};return n.integrity&&(s.integrity=n.integrity),n.referrerPolicy&&(s.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?s.credentials="include":n.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function r(n){if(n.ep)return;n.ep=!0;const s=o(n);fetch(n.href,s)}})();let O=class{constructor(e="https://lumina-production-a4bb.up.railway.app"){this.serverUrl=e,this.socket=null,this.connected=!1,this.currentDeviceId=null,this.eventHandlers=new Map}async connect(e,o=""){if(this.socket&&this.connected){console.warn("Already connected to WebSocket");return}return new Promise((r,n)=>{if(typeof io>"u"){const s=document.createElement("script");s.src="https://cdn.socket.io/4.5.4/socket.io.min.js",s.onload=()=>this._initializeSocket(e,o,r,n),s.onerror=()=>n(new Error("Failed to load Socket.IO")),document.head.appendChild(s)}else this._initializeSocket(e,o,r,n)})}_initializeSocket(e,o,r,n){try{this.socket=io(`${this.serverUrl}/control`,{auth:{userId:e,sessionToken:o},transports:["websocket","polling"]}),this.socket.on("connect",()=>{console.log("✅ Connected to ESP32 WebSocket server"),this.connected=!0,this._triggerEvent("connected"),r()}),this.socket.on("connect_error",s=>{console.error("❌ Connection error:",s.message),this.connected=!1,this._triggerEvent("error",s),n(s)}),this.socket.on("disconnect",()=>{console.log("❌ Disconnected from WebSocket server"),this.connected=!1,this._triggerEvent("disconnected")}),this.socket.on("device:status",s=>{console.log("📊 Device status:",s),this._triggerEvent("device:status",s)}),this.socket.on("device:telemetry",s=>{this._triggerEvent("device:telemetry",s)}),this.socket.on("command:response",s=>{this._triggerEvent("command:response",s)}),this.socket.on("command:sent",s=>{this._triggerEvent("command:sent",s)}),this.socket.on("device:error",s=>{console.error("Device error:",s),this._triggerEvent("device:error",s)}),this.socket.on("devices:list",s=>{this._triggerEvent("devices:list",s)}),this.socket.on("error",s=>{console.error("System error:",s),this._triggerEvent("error",s)})}catch(s){n(s)}}monitorDevice(e){if(!this.connected)throw new Error("Not connected to WebSocket server");this.currentDeviceId=e,this.socket.emit("monitor:device",e),console.log(`👁️ Monitoring device: ${e}`)}stopMonitoring(){this.currentDeviceId&&this.connected&&(this.socket.emit("monitor:stop",this.currentDeviceId),this.currentDeviceId=null)}sendCommand(e,o,r={}){if(!this.connected)throw new Error("Not connected to WebSocket server");this.socket.emit("control:command",{deviceId:e,command:o,payload:r}),console.log(`📤 Sent command to ${e}:`,o,r)}move(e,o,r=255){this.sendCommand(e,"move",{direction:o,speed:r})}stop(e){this.sendCommand(e,"stop")}turnLeft(e,o=200){this.sendCommand(e,"move",{direction:"left",speed:o})}turnRight(e,o=200){this.sendCommand(e,"move",{direction:"right",speed:o})}forward(e,o=255){this.sendCommand(e,"move",{direction:"forward",speed:o})}backward(e,o=255){this.sendCommand(e,"move",{direction:"backward",speed:o})}requestDeviceList(){if(!this.connected)throw new Error("Not connected to WebSocket server");this.socket.emit("devices:list")}on(e,o){this.eventHandlers.has(e)||this.eventHandlers.set(e,[]),this.eventHandlers.get(e).push(o)}off(e,o){if(!this.eventHandlers.has(e))return;const r=this.eventHandlers.get(e),n=r.indexOf(o);n>-1&&r.splice(n,1)}_triggerEvent(e,o=null){if(!this.eventHandlers.has(e))return;this.eventHandlers.get(e).forEach(n=>{try{n(o)}catch(s){console.error(`Error in event handler for ${e}:`,s)}})}disconnect(){this.socket&&(this.stopMonitoring(),this.socket.disconnect(),this.socket=null,this.connected=!1,console.log("👋 Disconnected from WebSocket"))}isConnected(){return this.connected}getCurrentDevice(){return this.currentDeviceId}};typeof $<"u"&&$.exports&&($.exports=O);typeof window<"u"&&(window.ESP32SocketClient=O);const N="https://lumina-production-a4bb.up.railway.app",U="http://localhost:3000",T=window.location.hostname==="jlfuertes14.github.io",E=T?`${N}/api`:`${U}/api`;async function w(t,e={}){const o=`${E}${t}`;try{const r=await fetch(o,{...e,headers:{"Content-Type":"application/json",...e.headers}}),n=await r.json();if(!r.ok)throw new Error(n.error||n.message||"API request failed");return n}catch(r){throw console.error("API Error:",r),r}}console.log(`🌍 Environment: ${T?"PRODUCTION":"DEVELOPMENT"}`);console.log(`🔗 API URL: ${E}`);const i={products:[],users:[],orders:[],currentUser:JSON.parse(localStorage.getItem("currentUser"))||null,cart:JSON.parse(localStorage.getItem("cart_v2"))||[],route:sessionStorage.getItem("currentRoute")||"home",mobileMenuOpen:!1,searchQuery:"",showSuggestions:!1,searchSuggestions:[],currentProductId:null,devices:[],currentDeviceId:null,esp32Client:null,deviceStatus:{},telemetryData:{},sortBy:"featured",filterCategory:null,cartSearchQuery:"",isLoading:!1,cartSynced:!1,checkoutData:{shipping:{fullName:"",address:"",city:"",province:"",postalCode:"",phone:"",instructions:""},paymentMethod:"cod",shippingFee:50},lastOrderId:null},b={getProducts:async()=>{try{const t=await w("/products");i.products=t.data,v()}catch(t){console.error("Failed to load products:",t),c("Failed to load products")}},login:async(t,e)=>{console.log("Calling API login...");try{const o=await w("/users/login",{method:"POST",body:JSON.stringify({email:t,password:e})});i.currentUser=o.data,localStorage.setItem("currentUser",JSON.stringify(i.currentUser)),await x.getMyDevices(),await F(),c(`Welcome back, ${i.currentUser.name}!`),g("home")}catch(o){throw c(o.message||"Login failed"),o}},register:async(t,e,o)=>{try{const r=await w("/users/register",{method:"POST",body:JSON.stringify({name:t,email:e,password:o})});i.currentUser=r.data,localStorage.setItem("currentUser",JSON.stringify(i.currentUser)),c("Account created successfully!"),g("home")}catch(r){throw c(r.message||"Registration failed"),r}},createOrder:async t=>{try{const e=await w("/orders",{method:"POST",body:JSON.stringify(t)});return i.cart=[],f(),c("Order placed successfully!"),i.currentUser.role==="admin"&&b.getOrders(),e.data}catch(e){throw c(e.message||"Failed to place order"),e}},getOrders:async()=>{if(!(!i.currentUser||i.currentUser.role!=="admin"))try{const t=await w("/orders");i.orders=t.data,v()}catch(t){console.error("Failed to load orders:",t)}},getUsers:async()=>{if(!(!i.currentUser||i.currentUser.role!=="admin"))try{const t=await w("/users");i.users=t.data,v()}catch(t){console.error("Failed to load users:",t)}}},x={getMyDevices:async()=>{if(i.currentUser)try{const t=await w(`/devices/my-devices?userId=${i.currentUser._id}`);i.devices=t.data,q(),v()}catch(t){console.error("Failed to load devices:",t),c("Failed to load devices")}},pairDevice:async(t,e)=>{try{const o=await w("/devices/pair",{method:"POST",body:JSON.stringify({deviceId:t,deviceToken:e,userId:i.currentUser._id})});c("Device paired successfully!"),await x.getMyDevices(),g("my-devices")}catch(o){throw c(o.message||"Device pairing failed"),o}}};function q(){i.esp32Client||(i.esp32Client=new ESP32SocketClient),!i.esp32Client.isConnected()&&i.currentUser&&i.esp32Client.connect(i.currentUser._id).catch(t=>{console.error("Failed to connect to WebSocket:",t)}),i.esp32Client.on("device:status",t=>{if(console.log("📊 Device status update:",t),i.devices){const e=i.devices.findIndex(o=>o.deviceId===t.deviceId);e!==-1&&(i.devices[e].status=t.status==="online"?"active":"offline",i.devices[e].lastOnline=t.timestamp,i.deviceStatus[t.deviceId]=t,i.route==="my-devices"&&v())}})}const f=()=>{localStorage.setItem("currentUser",JSON.stringify(i.currentUser)),localStorage.setItem("cart_v2",JSON.stringify(i.cart))},F=async()=>{if(!(!i.currentUser||i.cartSynced))try{const t=await w(`/users/${i.currentUser.id}/cart/sync`,{method:"POST",body:JSON.stringify({localCart:i.cart})});i.cart=t.data.map(e=>({id:e.productId,name:e.name,price:e.price,image:e.image,quantity:e.quantity,category:e.category,selected:e.selected})),i.cartSynced=!0,f(),v(),c("Cart synced!")}catch(t){console.error("Cart sync failed:",t)}},m=t=>new Intl.NumberFormat("en-PH",{style:"currency",currency:"PHP"}).format(t),g=t=>{i.route=t,i.mobileMenuOpen=!1,sessionStorage.setItem("currentRoute",t),v(),window.scrollTo(0,0)},c=t=>{const e=document.createElement("div");e.className="toast",e.textContent=t,document.body.appendChild(e),setTimeout(()=>e.classList.add("show"),100),setTimeout(()=>{e.classList.remove("show"),setTimeout(()=>e.remove(),300)},3e3)},k=t=>`
    ...
    `,H=()=>{const t=i.currentUser!==null,e=i.currentUser?.role==="admin",o=i.route==="cart",r=i.cart.length;return`
        <header>
                        ...
                        <div class="search-container">
                            ...
                            ${i.showSuggestions&&i.searchQuery?`
                                <div class="search-suggestions" id="searchSuggestions">
                                    ...
                                    ${i.searchSuggestions.slice(0,5).map(n=>`
    ...
    `},S=t=>{const e=t.stock<10,o=e?"low-stock":"",r=e?"Low Stock":"In Stock";return`
    ...
    `},L=()=>{if(i.searchQuery){const o=i.products.filter(r=>r.name.toLowerCase().includes(i.searchQuery.toLowerCase())||r.category.toLowerCase().includes(i.searchQuery.toLowerCase())||r.description.toLowerCase().includes(i.searchQuery.toLowerCase()));return`
            <div class="hero">
    ...
    `},G=()=>{if(!i.currentUser)return g("login"),"";const t=i.devices||[];return`
                    ...
                    ${t.map(e=>`
                        <div class="device-card">
                            ...
                            <p class="device-id" style="font-family: monospace; font-size: 0.85rem; color: var(--text-muted); margin-top: 0.25rem;">
                                🔑 Token: ${e.deviceToken?e.deviceToken.substring(0,16)+"...":"N/A"}
                            </p>
    ...
    `},K=()=>{if(!i.currentUser)return g("login"),"";const t=i.cart.filter(n=>n.selected!==!1);if(t.length===0)return c("No items selected for checkout"),g("cart"),"";const e=t.reduce((n,s)=>n+s.price*s.quantity,0),o=i.checkoutData.shippingFee,r=e+o;return`
            ...
            <div class="admin-section" style="margin-bottom: 2rem;">
                ...
                ${t.items.map(s=>`
                    <div style="display: flex; gap: 1rem; padding: 1rem 0; border-bottom: 1px solid var(--border);">
                        ...
                        <div style="font-weight: 700; color: var(--primary);">${m(s.price*s.quantity)}</div>
                    </div>
                `).join("")}
                <div style="display: flex; justify-content: space-between; font-size: 1.25rem; font-weight: 700; margin-top: 1rem; padding-top: 1rem; border-top: 2px solid var(--border);">
            ...
            <div style="max-width: 1400px; margin: 0 auto; padding: 0 2rem;">
                ...
                <div style="margin-bottom: 3rem;">
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 1.5rem;">
                        ${e.map(n=>{const s=r(n.category);return`
                            <div class="tutorial-card" data-category="${n.category}" data-topic="${n.topic}" 
                                 ...
                                 onclick="window.openVideoModal('${n.videoId}')"
                                ...
                                <div style="position: relative; aspect-ratio: 16/9; background: #000; overflow: hidden;">
                                    <img src="https://img.youtube.com/vi/${n.videoId}/maxresdefault.jpg" alt="${n.title}" style="width: 100%; height: 100%; object-fit: cover;" />
                                ...
                                <div style="padding: 1rem;">
                                    <h3 style="font-size: 1rem; margin-bottom: 0.5rem; line-height: 1.4; color: #1e293b; font-weight: 600; text-align: left;">${n.title}</h3>
                                    <p style="font-size: 0.875rem; color: #64748b; text-align: left;">Watch this ${n.category.toLowerCase()}-level tutorial</p>
                                </div>
                            </div>
                        `}).join("")}
                    </div>
                ...
                <div style="margin-bottom: 3rem;">
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 1.5rem;">
                        ${o.map(n=>{const s=r(n.category);return`
                            <div class="tutorial-card" data-category="${n.category}" data-topic="${n.topic}"
                                 style="background: white; border-radius: 8px; border: 1px solid #e2e8f0; overflow: hidden; cursor: pointer; transition: all 0.3s; box-shadow: 0 2px 8px rgba(0,0,0,0.08);" 
                                 onclick="window.openVideoModal('${n.videoId}')"
                                 onmouseover="this.style.transform='translateY(-4px)'; this.style.boxShadow='0 8px 16px rgba(0,0,0,0.12)'"
    ...
    `},ie=()=>{const t=i.products.slice(0,8).map((e,o)=>({...e,originalPrice:e.price*1.3,discount:[10,15,20,25,30][o%5],stock:[45,67,23,89,12][o%5],category:["Microcontrollers","Sensors","Tools","Robotics","Components"][o%5]}));return`
        <div style="background: #f8f9fa; min-height: 100vh; padding: 2rem 0;">
            <!-- Content Container -->
            <div style="max-width: 1400px; margin: 0 auto; padding: 0 2rem;">
                ${k("deals")}
    ...
    `};window.updateOrderStatus=(t,e)=>{const o=i.orders.find(r=>r.id===t);o&&(o.status=e,f(),c(`Order #${t} updated to ${e}`),v())};window.toggleSelectAll=t=>{document.querySelectorAll(".product-checkbox").forEach(o=>o.checked=t.checked)};window.bulkAction=t=>{const e=document.querySelectorAll(".product-checkbox:checked"),o=Array.from(e).map(r=>parseInt(r.value));if(o.length===0){c("No products selected");return}t==="delete"?confirm(`Delete ${o.length} products?`)&&(i.products=i.products.filter(r=>!o.includes(r.id)),f(),v(),c("Products deleted")):t==="restock"&&(i.products.forEach(r=>{o.includes(r.id)&&(r.stock+=10)}),f(),v(),c("Products restocked"))};window.viewOrderDetails=t=>{const e=i.orders.find(n=>n.id===t);if(!e)return;const o=i.users.find(n=>n.id===e.userId),r=`
                    ...
                    <tbody>
                        ${e.items.map(n=>`
                            <tr style="border-bottom: 1px solid var(--border);">
                                ...
                                <td style="padding: 0.5rem;">${m(n.price)}</td>
                            </tr>
                        `).join("")}
                    </tbody>
                </table>
    ...
    `;document.body.insertAdjacentHTML("beforeend",r)};window.closeModal=()=>{const t=document.getElementById("orderModal");t&&t.remove()};const oe=()=>{if(!i.currentUser||i.currentUser.role!=="admin")return g("home"),"";i.orders.length===0&&b.getOrders();const t=i.orders.reduce((a,u)=>a+u.total,0),e=i.orders.length;i.products.length;const o=i.users.filter(a=>a.role==="customer").length,r=i.products.filter(a=>a.stock<10);i.products.filter(a=>a.stock===0);const n=i.orders.slice(0,5),s=e>0?t/e:0,d={};i.products.forEach(a=>{d[a.category]=(d[a.category]||0)+1});const l=Object.entries(d).map(([a,u])=>({name:a,count:u})),p=[450,720,550,890,600,950,1200],h=Math.max(...p);return`
            ...
            <div class="analytics-grid">
                ...
                <div class="admin-section">
                    ...
                    <div class="chart-container">
                        ${p.map(a=>`
                            <div class="chart-bar" style="height: ${a/h*100}%" data-value="${m(a)}"></div>
                        `).join("")}
                    </div>
                </div>
            ...
            <div class="admin-section">
                ...
                <div class="table-container">
                    <table class="admin-table">
                        ...
                        <tbody>
                            ${n.length>0?n.map(a=>{const u=i.users.find(y=>y.id===a.userId);return`
                                    <tr>
                                        <td><strong>#${a.orderId}</strong></td>
                                        <td>${new Date(a.createdAt).toLocaleDateString()}</td>
                                        <td>${u?u.name:"Unknown (ID: "+a.userId+")"}</td>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
                    <div>
                        ...
                        <ul style="list-style: none; padding: 0; margin-top: 1rem;">
                            ${i.users.filter(a=>a.role==="customer").slice(0,3).map(a=>`
    ...
    `},re=(t=null)=>{const e=t?i.products.find(r=>r.id===t):null,o=!!e;return`
    ...
    `};window.navigate=g;window.toggleMobileMenu=()=>{i.mobileMenuOpen=!i.mobileMenuOpen,v()};window.showProductModal=(t=null)=>{document.body.insertAdjacentHTML("beforeend",re(t))};window.closeProductModal=()=>{document.getElementById("productModal")?.remove()};window.editProduct=t=>{window.showProductModal(t)};window.deleteProduct=async t=>{if(confirm("Delete this product?"))try{await w(`/products/${t}`,{method:"DELETE"}),await b.getProducts(),c("Product deleted")}catch{c("Delete failed")}};window.handleProductSubmit=async(t,e)=>{t.preventDefault();const o=new FormData(t.target);try{const r=e?"PUT":"POST",n=e?`/products/${e}`:"/products";if(!(await fetch(`${E}${n}`,{method:r,body:o})).ok)throw new Error("Failed");await b.getProducts(),window.closeProductModal(),c(e?"Product updated!":"Product created!")}catch{c("Operation failed")}};window.handleSort=t=>{i.sortBy=t,v()};window.viewProduct=t=>{i.currentProductId=t,g("product-detail")};window.adjustDetailQty=t=>{const e=document.getElementById("detailQty");let o=parseInt(e.value)+t;o<1&&(o=1),e.value=o};window.addToCartFromDetail=t=>{const e=parseInt(document.getElementById("detailQty").value);if(!i.currentUser){c("Please login to shop"),g("login");return}const o=i.products.find(n=>n.id===t),r=i.cart.find(n=>n.id===t);r?r.quantity+=e:i.cart.push({...o,quantity:e}),f(),c(`Added ${e} item(s) to cart`)};window.addToCart=async t=>{if(!i.currentUser){c("Please login to shop"),g("login");return}const e=i.products.find(r=>r.id===t),o=i.cart.find(r=>r.id===t);o?o.quantity+=1:i.cart.push({...e,quantity:1}),f(),v(),c("Added to cart"),i.currentUser&&await w(`/users/${i.currentUser.id}/cart`,{method:"PUT",body:JSON.stringify({cart:i.cart.map(r=>({productId:r.id,name:r.name,price:r.price,image:r.image,quantity:r.quantity,category:r.category,selected:r.selected!==!1}))})})};window.updateQuantity=(t,e)=>{if(e<1){window.removeFromCart(t);return}const o=i.cart.find(r=>r.id===t);o&&(o.quantity=e,f(),v())};window.removeFromCart=t=>{i.cart=i.cart.filter(e=>e.id!==t),f(),v()};window.checkout=async()=>{if(i.cart.length===0)return;if(!i.currentUser){c("Please login to checkout"),g("login");return}if(i.cart.filter(e=>e.selected!==!1).length===0){c("No items selected for checkout");return}g("checkout")};window.updateShippingInfo=(t,e)=>{i.checkoutData.shipping[t]=e};window.selectPaymentMethod=t=>{i.checkoutData.paymentMethod=t,v()};window.handlePhoneInput=t=>{const e=t.value.replace(/[^0-9]/g,"");t.value=e,i.checkoutData.shipping.phone=e};window.handleNameInput=t=>{const e=t.value.replace(/[^a-zA-Z\s]/g,"");t.value=e,i.checkoutData.shipping.fullName=e};window.handleLocationInput=(t,e)=>{const o=t.value.replace(/[^a-zA-Z\s]/g,"");t.value=o,i.checkoutData.shipping[e]=o};window.handlePostalInput=t=>{const e=t.value.replace(/[^0-9]/g,"");t.value=e,i.checkoutData.shipping.postalCode=e};window.showPaymentModal=t=>new Promise(e=>{let o="";const r=`
                ...
                ${t.items.map(s=>`
                    <div class="item-row">
                        ...
                        <div class="item-details">
                            ...
                            <div class="text-right"><strong>${m(s.price*s.quantity)}</strong></div>
                        </div>
                    </div>
                `).join("")}
            </div>
    ...
    `,n=window.open("","_blank","width=300,height=600");n.document.write(r),n.document.close(),n.onload=function(){setTimeout(()=>{n.print()},250)}};window.handleSearchInput=t=>{const e=t.target.value;if(i.searchQuery=e,e.trim()){const o=new Set;i.products.forEach(r=>{const n=r.name.toLowerCase(),s=r.category.toLowerCase(),d=e.toLowerCase();n.includes(d)&&o.add(r.name),s.includes(d)&&o.add(r.category),n.split(" ").forEach(p=>{p.toLowerCase().startsWith(d)&&p.length>2&&o.add(p.charAt(0).toUpperCase()+p.slice(1))})}),i.searchSuggestions=Array.from(o).slice(0,8),i.showSuggestions=!0}else if(i.searchSuggestions=[],i.showSuggestions=!1,i.route==="home"||i.route==="products"){v();return}A()};function A(){const t=document.querySelector(".search-container");if(!t)return;const e=t.querySelector(".search-suggestions");if(e&&e.remove(),i.showSuggestions&&i.searchQuery){const o=`
            <div class="search-suggestions" id="searchSuggestions">
                <div class="suggestions-header">Suggestions</div>
        ...
        `;t.insertAdjacentHTML("beforeend",o)}}window.showSearchSuggestions=()=>{i.searchQuery&&(i.showSuggestions=!0,A())};window.selectSuggestion=t=>{i.searchQuery=t,i.showSuggestions=!1;const e=document.getElementById("searchInput");e&&(e.value=t),handleSearch()};window.handleSearch=()=>{i.showSuggestions=!1;const t=document.getElementById("searchInput");t&&(i.searchQuery=t.value.trim()),g("products"),setTimeout(()=>{const e=document.querySelector(".product-grid");e&&e.scrollIntoView({behavior:"smooth",block:"start"})},100)};window.clearSearch=()=>{i.searchQuery="",i.showSuggestions=!1,i.searchSuggestions=[],v()};document.addEventListener("click",t=>{if(!t.target.closest(".search-container")&&i.showSuggestions){i.showSuggestions=!1;const e=document.querySelector(".search-suggestions");e&&e.remove()}});window.handleLogin=async t=>{console.log("Login attempt started"),t.preventDefault();const e=t.target.email.value,o=t.target.password.value;console.log("Credentials:",{email:e,password:o});try{await b.login(e,o),console.log("Login successful")}catch(r){console.error("Login error:",r)}};window.handleSignup=async t=>{console.log("Signup attempt started"),t.preventDefault();const e=t.target.name.value,o=t.target.email.value,r=t.target.password.value;try{await b.register(e,o,r)}catch(n){console.error("Signup error:",n)}};window.logout=()=>{i.currentUser=null,i.cart=[],localStorage.removeItem("currentUser"),localStorage.removeItem("cart_v2"),c("Logged out successfully"),sessionStorage.removeItem("currentRoute"),g("home")};window.handleContactSubmit=t=>{t.preventDefault();const e=t.target,o={name:e.name.value,email:e.email.value,subject:e.subject.value,message:e.message.value};console.log("Contact form submitted:",o),c("Thank you for your message! We will get back to you soon."),e.reset()};window.deleteProduct=t=>{confirm("Are you sure you want to remove this product?")&&(i.products=i.products.filter(e=>e.id!==t),f(),v(),c("Product removed"))};window.viewOrderDetails=t=>{const e=i.orders.find(d=>d.orderId===t);if(!e){c("Order not found");return}const o=i.users.find(d=>d.id===e.userId),r=o?o.name:`User ID: ${e.userId}`,n=document.createElement("div");n.className="order-details-modal",n.innerHTML=`
    ...
    `,document.body.appendChild(n);const s=d=>{d.key==="Escape"&&(n.remove(),document.removeEventListener("keydown",s))};document.addEventListener("keydown",s)};const v=()=>{const t=document.getElementById("app");let e="";switch(i.route){case"home":e=L();break;case"products":e=Q();break;case"product-detail":e=R();break;case"login":e=V();break;case"signup":e=W();break;case"cart":e=_();break;case"checkout":e=K();break;case"order-confirmation":e=X();break;case"contact-us":e=Z();break;case"about-us":e=ee();break;case"learn":e=te();break;case"deals":e=ie();break;case"admin":e=oe();break;case"my-devices":e=G();break;case"device-pair":e=Y();break;case"remote-control":e=J();break;default:e=L()}t.innerHTML=`
        ${H()}
        <main>
            ${e}
        </main>
        ...
        <div style="display: flex; flex-direction: column; background: var(--surface); border-bottom: 1px solid var(--border);">
            <div class="cart-item" style="border-bottom: none;">
                ...
                <div style="flex: 1;">
                    ...
                    <p class="text-muted">${m(s.price)}</p>
                </div>
                <div style="display: flex; align-items: center; gap: 1rem; margin-right: 2rem;">
