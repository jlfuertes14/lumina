(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))n(i);new MutationObserver(i=>{for(const o of i)if(o.type==="childList")for(const d of o.addedNodes)d.tagName==="LINK"&&d.rel==="modulepreload"&&n(d)}).observe(document,{childList:!0,subtree:!0});function s(i){const o={};return i.integrity&&(o.integrity=i.integrity),i.referrerPolicy&&(o.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?o.credentials="include":i.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function n(i){if(i.ep)return;i.ep=!0;const o=s(i);fetch(i.href,o)}})();const O="https://lumina-production-a4bb.up.railway.app",M="http://localhost:3000",L=window.location.hostname==="jlfuertes14.github.io",P=L?`${O}/api`:`${M}/api`;async function b(e,t={}){const s=`${P}${e}`;try{const n=await fetch(s,{...t,headers:{"Content-Type":"application/json",...t.headers}}),i=await n.json();if(!n.ok)throw new Error(i.error||i.message||"API request failed");return i}catch(n){throw console.error("API Error:",n),n}}console.log(`🌍 Environment: ${L?"PRODUCTION":"DEVELOPMENT"}`);console.log(`🔗 API URL: ${P}`);const a={products:[],users:[],orders:[],currentUser:JSON.parse(localStorage.getItem("currentUser"))||null,cart:JSON.parse(localStorage.getItem("cart_v2"))||[],route:sessionStorage.getItem("currentRoute")||"home",mobileMenuOpen:!1,searchQuery:"",showSuggestions:!1,searchSuggestions:[],currentProductId:null,sortBy:"featured",filterCategory:null,cartSearchQuery:"",isLoading:!1,cartSynced:!1,checkoutData:{shipping:{fullName:"",address:"",city:"",province:"",postalCode:"",phone:"",instructions:""},paymentMethod:"cod",shippingFee:50},lastOrderId:null},f={getProducts:async()=>{try{const e=await b("/products");a.products=e.data,g()}catch(e){console.error("Failed to load products:",e),c("Failed to load products")}},login:async(e,t)=>{console.log("Calling API login...");try{const s=await b("/users/login",{method:"POST",body:JSON.stringify({email:e,password:t})});a.currentUser=s.data,localStorage.setItem("currentUser",JSON.stringify(a.currentUser)),await E(),c(`Welcome back, ${a.currentUser.name}!`),y("home")}catch(s){throw c(s.message||"Login failed"),s}},register:async(e,t,s)=>{try{const n=await b("/users/register",{method:"POST",body:JSON.stringify({name:e,email:t,password:s})});a.currentUser=n.data,localStorage.setItem("currentUser",JSON.stringify(a.currentUser)),c("Account created successfully!"),y("home")}catch(n){throw c(n.message||"Registration failed"),n}},createOrder:async e=>{try{const t=await b("/orders",{method:"POST",body:JSON.stringify(e)});return a.cart=[],w(),c("Order placed successfully!"),a.currentUser.role==="admin"&&f.getOrders(),t.data}catch(t){throw c(t.message||"Failed to place order"),t}},getOrders:async()=>{if(!(!a.currentUser||a.currentUser.role!=="admin"))try{const e=await b("/orders");a.orders=e.data,g()}catch(e){console.error("Failed to load orders:",e)}},getUsers:async()=>{if(!(!a.currentUser||a.currentUser.role!=="admin"))try{const e=await b("/users");a.users=e.data,g()}catch(e){console.error("Failed to load users:",e)}}},w=()=>{localStorage.setItem("currentUser",JSON.stringify(a.currentUser)),localStorage.setItem("cart_v2",JSON.stringify(a.cart))},E=async()=>{if(!(!a.currentUser||a.cartSynced))try{const e=await b(`/users/${a.currentUser.id}/cart/sync`,{method:"POST",body:JSON.stringify({localCart:a.cart})});a.cart=e.data.map(t=>({id:t.productId,name:t.name,price:t.price,image:t.image,quantity:t.quantity,category:t.category,selected:t.selected})),a.cartSynced=!0,w(),g(),c("Cart synced!")}catch(e){console.error("Cart sync failed:",e)}},l=e=>new Intl.NumberFormat("en-PH",{style:"currency",currency:"PHP"}).format(e),y=e=>{a.route=e,a.mobileMenuOpen=!1,sessionStorage.setItem("currentRoute",e),g(),window.scrollTo(0,0)},c=e=>{const t=document.createElement("div");t.className="toast",t.textContent=e,document.body.appendChild(t),setTimeout(()=>t.classList.add("show"),100),setTimeout(()=>{t.classList.remove("show"),setTimeout(()=>t.remove(),300)},3e3)},B=()=>{const e=a.currentUser!==null,t=a.currentUser?.role==="admin",s=a.route==="cart",n=a.cart.length;return`
        <header>
    ...
    `},k=e=>{const t=e.stock<10,s=t?"low-stock":"",n=t?"Low Stock":"In Stock";return`
    ...
    `},T=()=>{const e=a.products.find(i=>i.id===a.currentProductId);if(!e)return y("home"),"";e.stock<10;const t=e.stock>0?"In Stock":"Out of Stock",s=e.stock>0?"var(--success)":"var(--danger)",n=a.products.filter(i=>i.id!==e.id).sort(()=>.5-Math.random()).slice(0,4);return`
    ...
    `},I=()=>{if(a.searchQuery){const s=a.products.filter(n=>n.name.toLowerCase().includes(a.searchQuery.toLowerCase())||n.category.toLowerCase().includes(a.searchQuery.toLowerCase())||n.description.toLowerCase().includes(a.searchQuery.toLowerCase()));return`
            <div class="hero">
        ...
        <div style="padding: 2rem 0;">
            ...
            <div class="product-grid">
                ${a.products.filter(s=>e.includes(s.id)).sort((s,n)=>e.indexOf(s.id)-e.indexOf(n.id)).map(k).join("")}
            </div>
        </div>
    `},N=()=>{let e=[...a.products];switch(a.searchQuery&&(e=e.filter(t=>t.name.toLowerCase().includes(a.searchQuery.toLowerCase())||t.category.toLowerCase().includes(a.searchQuery.toLowerCase())||t.description.toLowerCase().includes(a.searchQuery.toLowerCase()))),a.sortBy){case"price-asc":e.sort((t,s)=>t.price-s.price);break;case"price-desc":e.sort((t,s)=>s.price-t.price);break;case"name-asc":e.sort((t,s)=>t.name.localeCompare(s.name));break;case"name-desc":e.sort((t,s)=>s.name.localeCompare(t.name));break;case"featured":default:e.sort((t,s)=>t.id-s.id);break}return`
    ...
    `,q=()=>{if(a.cart.length===0)return`
        ...
        <div style="margin: 2rem auto; padding: 0 2rem;">
            ...
            <div class="cart-items">
                ${e.map(t=>{const s=a.products.filter(n=>n.category===t.category&&n.id!==t.id).slice(0,4);return`
    ...
    `},U=()=>{if(!a.currentUser)return y("login"),"";const e=a.cart.filter(i=>i.selected!==!1);if(e.length===0)return c("No items selected for checkout"),y("cart"),"";const t=e.reduce((i,o)=>i+o.price*o.quantity,0),s=a.checkoutData.shippingFee,n=t+s;return`
                    ...
                    <div class="admin-section">
                        ...
                        <div style="display: grid; grid-template-columns: repeat(5, 1fr); gap: 0.75rem;">
                            ${[{id:"cod",label:"Cash on Delivery",image:"/lumina/images/payment/cod.png"},{id:"gcash",label:"GCash",image:"/lumina/images/payment/gcash.png"},{id:"maya",label:"Maya",image:"/lumina/images/payment/maya.png"},{id:"card",label:"Credit/Debit Card",image:"/lumina/images/payment/card.png"},{id:"bank",label:"Bank Transfer",image:"/lumina/images/payment/bank.png"}].map(i=>`
                                <div class="payment-method-card" onclick="window.selectPaymentMethod('${i.id}')" style="padding: 0.75rem; border: 2px solid ${a.checkoutData.paymentMethod===i.id?"var(--primary)":"var(--border)"}; border-radius: var(--radius-md); cursor: pointer; text-align: center; transition: all 0.2s; background: ${a.checkoutData.paymentMethod===i.id?"rgba(0, 43, 91, 0.05)":"var(--surface)"};">
    ...
    `},Q=()=>{if(!a.lastOrderId)return y("home"),"";const e=a.orders.find(o=>o.orderId===a.lastOrderId);if(!e)return y("home"),"";const t=new Date,s=new Date(t);s.setDate(t.getDate()+3);const n=new Date(t);n.setDate(t.getDate()+5);const i=`${s.toLocaleDateString("en-US",{month:"short",day:"numeric"})} - ${n.toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"})} `;return`
    ...
    `};window.updateOrderStatus=(e,t)=>{const s=a.orders.find(n=>n.id===e);s&&(s.status=t,w(),c(`Order #${e} updated to ${t}`),g())};window.toggleSelectAll=e=>{document.querySelectorAll(".product-checkbox").forEach(s=>s.checked=e.checked)};window.bulkAction=e=>{const t=document.querySelectorAll(".product-checkbox:checked"),s=Array.from(t).map(n=>parseInt(n.value));if(s.length===0){c("No products selected");return}e==="delete"?confirm(`Delete ${s.length} products?`)&&(a.products=a.products.filter(n=>!s.includes(n.id)),w(),g(),c("Products deleted")):e==="restock"&&(a.products.forEach(n=>{s.includes(n.id)&&(n.stock+=10)}),w(),g(),c("Products restocked"))};window.viewOrderDetails=e=>{const t=a.orders.find(i=>i.id===e);if(!t)return;const s=a.users.find(i=>i.id===t.userId),n=`
    ...
    `;document.body.insertAdjacentHTML("beforeend",n)};window.closeModal=()=>{const e=document.getElementById("orderModal");e&&e.remove()};const z=()=>{if(!a.currentUser||a.currentUser.role!=="admin")return y("home"),"";a.orders.length===0&&f.getOrders();const e=a.orders.reduce((r,m)=>r+m.total,0),t=a.orders.length;a.products.length;const s=a.users.filter(r=>r.role==="customer").length,n=a.products.filter(r=>r.stock<10);a.products.filter(r=>r.stock===0);const i=a.orders.slice(0,5),o=t>0?e/t:0,d={};a.products.forEach(r=>{d[r.category]=(d[r.category]||0)+1});const u=Object.entries(d).map(([r,m])=>({name:r,count:m})),p=[450,720,550,890,600,950,1200],v=Math.max(...p);return`
            ...
            <div class="admin-section">
                ...
                <div class="table-container">
                    <table class="admin-table">
                        ...
                        <tbody>
                            ${i.length>0?i.map(r=>{const m=a.users.find(h=>h.id===r.userId);return`
                                    <tr>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
                    <div>
                        ...
                        <ul style="list-style: none; padding: 0; margin-top: 1rem;">
                            ${a.users.filter(r=>r.role==="customer").slice(0,3).map(r=>`
    ...
    `},R=(e=null)=>{const t=e?a.products.find(n=>n.id===e):null,s=!!t;return`
    ...
    `};window.navigate=y;window.toggleMobileMenu=()=>{a.mobileMenuOpen=!a.mobileMenuOpen,g()};window.showProductModal=(e=null)=>{document.body.insertAdjacentHTML("beforeend",R(e))};window.closeProductModal=()=>{document.getElementById("productModal")?.remove()};window.editProduct=e=>{window.showProductModal(e)};window.deleteProduct=async e=>{if(confirm("Delete this product?"))try{await b(`/products/${e}`,{method:"DELETE"}),await f.getProducts(),c("Product deleted")}catch{c("Delete failed")}};window.handleProductSubmit=async(e,t)=>{e.preventDefault();const s=new FormData(e.target);try{const n=t?"PUT":"POST",i=t?`/products/${t}`:"/products";if(!(await fetch(`${P}${i}`,{method:n,body:s})).ok)throw new Error("Failed");await f.getProducts(),window.closeProductModal(),c(t?"Product updated!":"Product created!")}catch{c("Operation failed")}};window.handleSort=e=>{a.sortBy=e,g()};window.viewProduct=e=>{a.currentProductId=e,y("product-detail")};window.adjustDetailQty=e=>{const t=document.getElementById("detailQty");let s=parseInt(t.value)+e;s<1&&(s=1),t.value=s};window.addToCartFromDetail=e=>{const t=parseInt(document.getElementById("detailQty").value);if(!a.currentUser){c("Please login to shop"),y("login");return}const s=a.products.find(i=>i.id===e),n=a.cart.find(i=>i.id===e);n?n.quantity+=t:a.cart.push({...s,quantity:t}),w(),c(`Added ${t} item(s) to cart`)};window.addToCart=async e=>{if(!a.currentUser){c("Please login to shop"),y("login");return}const t=a.products.find(n=>n.id===e),s=a.cart.find(n=>n.id===e);s?s.quantity+=1:a.cart.push({...t,quantity:1}),w(),g(),c("Added to cart"),a.currentUser&&await b(`/users/${a.currentUser.id}/cart`,{method:"PUT",body:JSON.stringify({cart:a.cart.map(n=>({productId:n.id,name:n.name,price:n.price,image:n.image,quantity:n.quantity,category:n.category,selected:n.selected!==!1}))})})};window.updateQuantity=(e,t)=>{if(t<1){window.removeFromCart(e);return}const s=a.cart.find(n=>n.id===e);s&&(s.quantity=t,w(),g())};window.removeFromCart=e=>{a.cart=a.cart.filter(t=>t.id!==e),w(),g()};window.checkout=async()=>{if(a.cart.length===0)return;if(!a.currentUser){c("Please login to checkout"),y("login");return}if(a.cart.filter(t=>t.selected!==!1).length===0){c("No items selected for checkout");return}y("checkout")};window.updateShippingInfo=(e,t)=>{a.checkoutData.shipping[e]=t};window.selectPaymentMethod=e=>{a.checkoutData.paymentMethod=e,g()};window.handlePhoneInput=e=>{const t=e.value.replace(/[^0-9]/g,"");e.value=t,a.checkoutData.shipping.phone=t};window.handleNameInput=e=>{const t=e.value.replace(/[^a-zA-Z\s]/g,"");e.value=t,a.checkoutData.shipping.fullName=t};window.handleLocationInput=(e,t)=>{const s=e.value.replace(/[^a-zA-Z\s]/g,"");e.value=s,a.checkoutData.shipping[t]=s};window.handlePostalInput=e=>{const t=e.value.replace(/[^0-9]/g,"");e.value=t,a.checkoutData.shipping.postalCode=t};window.showPaymentModal=e=>new Promise(t=>{let s="";const n=`
        ...
        `;document.body.insertAdjacentHTML("beforeend",n);const i=document.getElementById("paymentModalOverlay"),o=document.getElementById("paymentConfirmBtn"),d=document.getElementById("paymentCancelBtn"),u=document.getElementById("processingOverlay"),p=document.getElementById("successOverlay");d.addEventListener("click",()=>{i.remove(),t(null)}),o.addEventListener("click",()=>{u.classList.add("show"),setTimeout(()=>{u.classList.remove("show"),p.classList.add("show"),setTimeout(()=>{i.remove(),t({method:"gcash",reference:s})},1e3)},1500)}),i.addEventListener("click",v=>{v.target===i&&(i.remove(),t(null))})});window.showMayaModal=e=>new Promise(t=>{const s="MAYA-"+Date.now().toString().slice(-8),n=`
            <div class="payment-modal-overlay" id="paymentModalOverlay">
                <div class="payment-modal">
                    <div class="demo-badge">DEMO MODE</div>
                    <div class="payment-modal-header">
        ...
        `;document.body.insertAdjacentHTML("beforeend",n);const i=document.getElementById("paymentModalOverlay"),o=document.getElementById("paymentConfirmBtn"),d=document.getElementById("paymentCancelBtn"),u=document.getElementById("processingOverlay"),p=document.getElementById("successOverlay");d.addEventListener("click",()=>{i.remove(),t(null)}),o.addEventListener("click",()=>{u.classList.add("show"),setTimeout(()=>{u.classList.remove("show"),p.classList.add("show"),setTimeout(()=>{i.remove(),t({method:"maya",reference:s})},1e3)},1500)}),i.addEventListener("click",v=>{v.target===i&&(i.remove(),t(null))})});window.showCardModal=e=>new Promise(t=>{const d=`
            <div class="payment-modal-overlay" id="paymentModalOverlay">
                <div class="payment-modal">
                    <div class="demo-badge">DEMO MODE</div>
                    <div class="payment-modal-header">
        ...
        `;document.body.insertAdjacentHTML("beforeend",d);const u=document.getElementById("paymentModalOverlay"),p=document.getElementById("paymentConfirmBtn"),v=document.getElementById("paymentCancelBtn"),r=document.getElementById("processingOverlay"),m=document.getElementById("successOverlay");v.addEventListener("click",()=>{u.remove(),t(null)}),p.addEventListener("click",()=>{r.classList.add("show"),setTimeout(()=>{r.classList.remove("show"),m.classList.add("show"),setTimeout(()=>{u.remove(),t({method:"card",last4:"1111"})},1e3)},2e3)}),u.addEventListener("click",h=>{h.target===u&&(u.remove(),t(null))})});window.showBankModal=e=>new Promise(t=>{const s="BDO-"+Date.now().toString().slice(-8),n=`
            <div class="payment-modal-overlay" id="paymentModalOverlay">
                <div class="payment-modal">
                    <div class="demo-badge">DEMO MODE</div>
                    <div class="payment-modal-header">
        ...
        `;document.body.insertAdjacentHTML("beforeend",n);const i=document.getElementById("paymentModalOverlay"),o=document.getElementById("paymentConfirmBtn"),d=document.getElementById("paymentCancelBtn"),u=document.getElementById("processingOverlay"),p=document.getElementById("successOverlay");d.addEventListener("click",()=>{i.remove(),t(null)}),o.addEventListener("click",()=>{u.classList.add("show"),setTimeout(()=>{u.classList.remove("show"),p.classList.add("show"),setTimeout(()=>{i.remove(),t({method:"bank",reference:s})},1e3)},1500)}),i.addEventListener("click",v=>{v.target===i&&(i.remove(),t(null))})});window.placeOrder=async()=>{const e=a.checkoutData.shipping;if(!e.fullName||!e.fullName.trim()){c("Please enter your full name");return}if(!e.address||!e.address.trim()){c("Please enter your address");return}if(!e.city||!e.city.trim()){c("Please enter your city");return}if(!e.province||!e.province.trim()){c("Please enter your province");return}if(!e.phone||!e.phone.trim()){c("Please enter your phone number");return}if(!/^09\d{9}$/.test(e.phone)){c('Phone number must start with "09" and contain exactly 11 digits');return}const s=/^[a-zA-Z\s]+$/;if(!s.test(e.fullName)){c("Full Name must contain letters and spaces only");return}if(!s.test(e.city)){c("City must contain letters and spaces only");return}if(!s.test(e.province)){c("Province must contain letters and spaces only");return}if(e.postalCode&&!/^\d+$/.test(e.postalCode)){c("Postal Code must contain numbers only");return}if(!a.checkoutData.paymentMethod){c("Please select a payment method");return}const n=a.cart.filter(r=>r.selected!==!1),o=n.reduce((r,m)=>r+m.price*m.quantity,0)+a.checkoutData.shippingFee;let d=null;switch(a.checkoutData.paymentMethod){case"cod":d=await showPaymentModal(o);break;case"gcash":d=await showGCashModal(o);break;case"maya":d=await showMayaModal(o);break;case"card":d=await showCardModal(o);break;case"bank":d=await showBankModal(o);break;default:c("Please select a payment method");return}if(!d)return;let u=d.amountPaid||o,p=d.change||0;const v={userId:a.currentUser.id,items:n.map(r=>({productId:r.id,quantity:r.quantity,price:r.price,name:r.name})),total:o,shippingInfo:a.checkoutData.shipping,paymentMethod:a.checkoutData.paymentMethod,shippingFee:a.checkoutData.shippingFee,amountPaid:u,change:p};try{const r=await f.createOrder(v);a.lastOrderId=r.orderId,a.lastOrderPayment={amountPaid:u,change:p},a.orders.push({orderId:r.orderId,...r,items:n,total:o,createdAt:new Date().toISOString(),userId:a.currentUser.id}),a.cart=a.cart.filter(m=>m.selected===!1),w(),y("order-confirmation")}catch{}};window.printReceipt=()=>{if(!a.lastOrderId){c("No order found to print");return}const e=a.orders.find(o=>o.orderId===a.lastOrderId);if(!e){c("Order not found");return}const t=a.lastOrderPayment||{},s=new Date,n=`
        <!DOCTYPE html>
        <html>
        <head>
            <meta charset="UTF-8">
    ...
    `,i=window.open("","_blank","width=300,height=600");i.document.write(n),i.document.close(),i.onload=function(){setTimeout(()=>{i.print()},250)}};window.handleSearchInput=e=>{const t=e.target.value;if(a.searchQuery=t,t.trim()){const s=new Set;a.products.forEach(n=>{const i=n.name.toLowerCase(),o=n.category.toLowerCase(),d=t.toLowerCase();i.includes(d)&&s.add(n.name),o.includes(d)&&s.add(n.category),i.split(" ").forEach(p=>{p.toLowerCase().startsWith(d)&&p.length>2&&s.add(p.charAt(0).toUpperCase()+p.slice(1))})}),a.searchSuggestions=Array.from(s).slice(0,8),a.showSuggestions=!0}else if(a.searchSuggestions=[],a.showSuggestions=!1,a.route==="home"||a.route==="products"){g();return}D()};function D(){const e=document.querySelector(".search-container");if(!e)return;const t=e.querySelector(".search-suggestions");if(t&&t.remove(),a.showSuggestions&&a.searchQuery){const s=`
            <div class="search-suggestions" id="searchSuggestions">
                <div class="suggestions-header">Suggestions</div>
                ${a.searchSuggestions.slice(0,5).map(n=>`
        ...
        `;e.insertAdjacentHTML("beforeend",s)}}window.showSearchSuggestions=()=>{a.searchQuery&&(a.showSuggestions=!0,D())};window.selectSuggestion=e=>{a.searchQuery=e,a.showSuggestions=!1;const t=document.getElementById("searchInput");t&&(t.value=e),handleSearch()};window.handleSearch=()=>{a.showSuggestions=!1;const e=document.getElementById("searchInput");e&&(a.searchQuery=e.value.trim()),y("products"),setTimeout(()=>{const t=document.querySelector(".product-grid");t&&t.scrollIntoView({behavior:"smooth",block:"start"})},100)};window.clearSearch=()=>{a.searchQuery="",a.showSuggestions=!1,a.searchSuggestions=[],g()};document.addEventListener("click",e=>{if(!e.target.closest(".search-container")&&a.showSuggestions){a.showSuggestions=!1;const t=document.querySelector(".search-suggestions");t&&t.remove()}});window.handleLogin=async e=>{console.log("Login attempt started"),e.preventDefault();const t=e.target.email.value,s=e.target.password.value;console.log("Credentials:",{email:t,password:s});try{await f.login(t,s),console.log("Login successful")}catch(n){console.error("Login error:",n)}};window.handleSignup=async e=>{console.log("Signup attempt started"),e.preventDefault();const t=e.target.name.value,s=e.target.email.value,n=e.target.password.value;try{await f.register(t,s,n)}catch(i){console.error("Signup error:",i)}};window.logout=()=>{a.currentUser=null,a.cart=[],localStorage.removeItem("currentUser"),localStorage.removeItem("cart_v2"),c("Logged out successfully"),sessionStorage.removeItem("currentRoute"),y("home")};window.deleteProduct=e=>{confirm("Are you sure you want to remove this product?")&&(a.products=a.products.filter(t=>t.id!==e),w(),g(),c("Product removed"))};window.viewOrderDetails=e=>{const t=a.orders.find(d=>d.orderId===e);if(!t){c("Order not found");return}const s=a.users.find(d=>d.id===t.userId),n=s?s.name:`User ID: ${t.userId}`,i=document.createElement("div");i.className="order-details-modal",i.innerHTML=`
    ...
    `,document.body.appendChild(i);const o=d=>{d.key==="Escape"&&(i.remove(),document.removeEventListener("keydown",o))};document.addEventListener("keydown",o)};const g=()=>{const e=document.getElementById("app");let t="";switch(a.route){case"home":t=I();break;case"products":t=N();break;case"product-detail":t=T();break;case"login":t=A();break;case"signup":t=j();break;case"cart":t=q();break;case"checkout":t=U();break;case"order-confirmation":t=Q();break;case"admin":t=z();break;default:t=I()}e.innerHTML=`
        ${B()}
        <main>
            ${t}
        </main>
    ...
    `};window.handleCartSearch=e=>{a.cartSearchQuery=e.target.value,$()};window.clearCartSearch=()=>{a.cartSearchQuery="",$()};window.toggleCartItem=e=>{const t=a.cart.find(s=>s.id===e);t&&(t.selected=t.selected===!1,w(),$())};window.toggleFindSimilar=e=>{const t=a.cart.find(s=>s.id===e);t&&(a.cart.forEach(s=>{s.id!==e&&(s.showSimilar=!1)}),t.showSimilar=!t.showSimilar,w(),$())};function $(){const e=document.querySelector(".cart-items"),t=document.querySelector(".cart-summary"),s=document.querySelector(".cart-search-message");if(!e)return;let n=a.cart;if(a.cartSearchQuery&&(n=a.cart.filter(o=>o.name.toLowerCase().includes(a.cartSearchQuery.toLowerCase())||o.category.toLowerCase().includes(a.cartSearchQuery.toLowerCase()))),e.innerHTML=n.map(o=>{const d=a.products.filter(u=>u.category===o.category&&u.id!==o.id).slice(0,4);return`
        <div style="display: flex; flex-direction: column; background: var(--surface); border-bottom: 1px solid var(--border);">
            <div class="cart-item" style="border-bottom: none;">
                <input type="checkbox" 
                    style="width: 20px; height: 20px; margin-right: 1rem; cursor: pointer; accent-color: var(--primary);"
    ...
    `}).join(""),t){const o=a.cart.reduce((u,p)=>u+(p.selected!==!1?p.price*p.quantity:0),0),d=a.cart.filter(u=>u.selected!==!1).length;t.innerHTML=`
            <div style="display: flex; justify-content: space-between; margin-bottom: 1rem; font-size: 1.25rem; font-weight: 700;">
                <span>Total</span>
                <span>${l(o)}</span>
            </div>
        ...
        `}s&&(a.cartSearchQuery&&n.length===0?(s.innerHTML=`
                <p style="color: var(--text-muted); margin-bottom: 1rem; text-align: center;">
                    No items found for "${a.cartSearchQuery}"
                </p>
            `,s.style.display="block"):s.style.display="none");const i=document.querySelector("#cartSearchInput + .search-btn");i&&(i.innerHTML=a.cartSearchQuery?`
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <line x1="18" y1="6" x2="6" y2="18"></line>
                <line x1="6" y1="6" x2="18" y2="18"></line>
            </svg>
        ...
        `)}const F=async()=>{await f.getProducts(),a.currentUser?.role==="admin"&&await Promise.all([f.getOrders(),f.getUsers()]),g()};F();
